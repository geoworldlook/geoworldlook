    ...
    map.on('load', () => {
      ...
      map.on('click', 'vineyard-fill', async (e) => {
        ...
        if (blockBase) {
          ...
          // Simple centroid estimation for flyTo
          if (blockBase.geom.type === 'Polygon') {
            const coords = blockBase.geom.coordinates[0];
            const lng = coords.reduce((acc: number, c: any) => acc + c[0], 0) / coords.length;
            const lat = coords.reduce((acc: number, c: any) => acc + c[1], 0) / coords.length;
