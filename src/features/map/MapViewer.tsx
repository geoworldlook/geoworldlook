...
export default function MapViewer({ points }: MapViewerProps) {
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      // Create GeoJSON source for vineyard blocks
      ...
      if (blocks.length > 0) {
          ...
          blocks.forEach(b => {
              if (b.geom && b.geom.coordinates) {
                  const coords = b.geom.type === 'Polygon' ? b.geom.coordinates[0] : [];
      ...
      map.on('click', 'vineyard-fill', async (e) => {
        ...
        if (blockBase) {
          ...
          // Fly to block center
          const bounds = new maplibregl.LngLatBounds();
          const coords = blockBase.geom.type === 'Polygon' ? blockBase.geom.coordinates[0] : [];
          coords.forEach((coord: [number, number]) => bounds.extend(coord));
