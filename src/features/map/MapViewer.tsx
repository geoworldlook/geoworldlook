...
export default function MapViewer({ points }: MapViewerProps) {
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      // Fit bounds if we have blocks
      if (blocks.length > 0) {
        const bounds = new maplibregl.LngLatBounds();
        blocks.forEach(block => {
          if (block.geom.type === 'Polygon') {
            block.geom.coordinates[0].forEach((coord: any) => {
      ...
      // 1. Add Vineyard Polygons Source
      ...
      map.on('click', 'vineyard-fill', async (e) => {
        ...
        if (blockBase) {
          ...
          // Calculate bounds of the polygon for centering
          if (blockBase.geom.type === 'Polygon') {
             const bounds = new maplibregl.LngLatBounds();
             blockBase.geom.coordinates[0].forEach((coord: any) => {
               bounds.extend(coord as [number, number]);
             });
