...
export default function MapViewer({ points }: MapViewerProps) {
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      // Fit bounds to all blocks on load
      if (blocks.length > 0) {
        const allBounds = new maplibregl.LngLatBounds();
        blocks.forEach(b => {
            const coords = (b.geom as any).coordinates[0];
            coords.forEach((coord: number[]) => allBounds.extend(coord as [number, number]));
        });
        map.fitBounds(allBounds, { padding: 50, maxZoom: 14 });
      }
    })
