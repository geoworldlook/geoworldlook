  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      map.addSource('vineyard-blocks', {
        ...
        data: {
          ...
          features: blocks.map((b, index) => ({
            type: 'Feature',
            id: index, // MapLibre needs a top-level id for setFeatureState
            geometry: b.geom,
            properties: {
              id: b.id,
              name: b.name,
      ...
      map.on('click', 'vineyard-fill', async (e) => {
        ...
        const blockBase = blocks.find(b => b.id === blockProps.id)
        
        if (blockBase) {
          const stats = await getBlockStats(blockBase.id);
      ...
      // Automatic fit to bounds if there are blocks
      if (blocks.length > 0) {
