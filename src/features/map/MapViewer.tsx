...
interface MapViewerProps {
  points?: any[] // Keep for compatibility but prioritize vineyard blocks
}
...
export default function MapViewer({ points }: MapViewerProps) {
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      // Adjust map view to fit all blocks
      if (blocks.length > 0) {
      ...
      map.on('click', 'vineyard-fill', async (e) => {
        ...
        if (blockBase) {
          ...
          if (blockBase.geom.type === 'Polygon') {
            blockBase.geom.coordinates[0].forEach((coord: [number, number]) => {
              bounds.extend(coord);
            });
            map.flyTo({
