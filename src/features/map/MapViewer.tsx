...
export default function MapViewer({ initialData }: MapViewerProps) {
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      // Create GeoJSON features from blocks
      const features = blocks.map(b => {
        let geometry = b.geom;
        // PostGIS geom can be a string (GeoJSON) or object depending on driver
      ...
      map.addSource('vineyard-blocks', {
        ...
        // Important for feature-state hover to work
        generateId: true
      })
      ...
      // If we have blocks, fit map to them
