...
export default function MapViewer({ points }: MapViewerProps) {
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      // Fit bounds to the blocks if they exist
      if (blocks.length > 0) {
        const bounds = new maplibregl.LngLatBounds();
        blocks.forEach(b => {
          // Recursive function to handle nested arrays in Polygon/MultiPolygon
      ...
      map.addSource('vineyard-blocks', {
        ...
        generateId: true // Generate numeric IDs for feature-state
      })
      ...
      map.on('click', 'vineyard-fill', async (e) => {
        ...
        // Use properties.id for business logic
        ...
        if (blockBase) {
          ...
          // Simple centroid calculation for better UX
