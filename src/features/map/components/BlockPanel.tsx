...
export default function BlockPanel({ block, onClose }: BlockPanelProps) {
  ...
  const hasStats = block.stats && block.stats.length > 0;
  const latestData = hasStats ? block.stats[block.stats.length - 1] : null;

  const chartData = hasStats ? block.stats.map(d => ({
    ...d,
        ...
        {latestData ? (
          <>
            <div className="grid grid-cols-2 gap-4">
              <div
                className={`p-3 rounded-lg border cursor-pointer transition-colors ${activeMetric === 'ndvi' ? 'bg-emerald-500/10 border-emerald-500/50' : 'bg-white/[0.03] border-white/[0.05] hover:bg-white/[0.05]'}`}
              ...
              <div
                className={`p-3 rounded-lg border cursor-pointer transition-colors ${activeMetric === 'ndmi' ? 'bg-blue-500/10 border-blue-500/50' : 'bg-white/[0.03] border-white/[0.05] hover:bg-white/[0.05]'}`}
