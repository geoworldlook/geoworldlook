      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          ...
          >
            ...
            <p className={`text-2xl font-bold ${activeTab === 'ndvi' ? 'text-emerald-400' : 'text-gray-400'}`}>
              {latestData?.ndvi_mean.toFixed(2) || 'N/A'}
            </p>
          </div>
          <div
            className={`cursor-pointer p-3 rounded-lg border transition-all ${activeTab === 'ndmi' ? 'bg-sky-500/10 border-sky-500/50' : 'bg-white/[0.03] border-white/[0.05]'}`}
            onClick={() => setActiveTab('ndmi')}
          >
            <p className="text-[10px] text-gray-500 uppercase mb-1 flex items-center gap-1">
              <Droplets className="w-3 h-3" /> NDMI
            </p>
            <p className={`text-2xl font-bold ${activeTab === 'ndmi' ? 'text-sky-400' : 'text-gray-400'}`}>
