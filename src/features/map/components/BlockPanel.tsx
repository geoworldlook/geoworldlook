...
export default function BlockPanel({ block, onClose }: BlockPanelProps) {
  const [activeTab, setActiveTab] = useState<'all' | 'ndvi' | 'ndmi'>('all');
    ...
    <Card className="absolute top-4 right-4 w-80 md:w-96 shadow-2xl border-white/[0.1] bg-black/90 backdrop-blur-md animate-in slide-in-from-right duration-300 z-[1000]">
      <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
        <div>
          ...
          <p className="text-xs text-gray-500">{block.area_ha} ha · Sentinel-2 Analysis</p>
        </div>
          ...
          <>
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`text-[10px] px-2 py-1 rounded border ${activeTab === 'all' ? 'bg-emerald-500/10 border-emerald-500/50 text-emerald-400' : 'border-white/10 text-gray-500'}`}
              >
                ALL
              </button>
              <button
                onClick={() => setActiveTab('ndvi')}
                className={`text-[10px] px-2 py-1 rounded border ${activeTab === 'ndvi' ? 'bg-emerald-500/10 border-emerald-500/50 text-emerald-400' : 'border-white/10 text-gray-500'}`}
              >
                NDVI
              </button>
              <button
                onClick={() => setActiveTab('ndmi')}
                className={`text-[10px] px-2 py-1 rounded border ${activeTab === 'ndmi' ? 'bg-sky-500/10 border-sky-500/50 text-sky-400' : 'border-white/10 text-gray-500'}`}
              >
                NDMI
              </button>
            </div>
