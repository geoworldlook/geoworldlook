  ...
  if (!latestData) {
      return (
        <Card className="absolute top-4 right-4 w-80 md:w-96 shadow-2xl border-white/[0.1] bg-black/90 backdrop-blur-md animate-in slide-in-from-right duration-300 z-[1000]">
          ...
          <CardContent>
            <p className="text-sm text-gray-400">No satellite data available for this block yet.</p>
          </CardContent>
        </Card>
      );
  }
    ...
    <Card className="absolute top-4 right-4 w-80 md:w-96 shadow-2xl border-white/[0.1] bg-black/90 backdrop-blur-md animate-in slide-in-from-right duration-300 z-[1000]">
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div
            className={`cursor-pointer bg-white/[0.03] p-3 rounded-lg border transition-colors ${activeMetric === 'ndvi' ? 'border-emerald-500/50 bg-emerald-500/5' : 'border-white/[0.05]'}`}
          ...
          <div
            className={`cursor-pointer bg-white/[0.03] p-3 rounded-lg border transition-colors ${activeMetric === 'ndmi' ? 'border-blue-500/50 bg-blue-500/5' : 'border-white/[0.05]'}`}
        ...
        <div className="bg-white/[0.03] p-3 rounded-lg border border-white/[0.05] flex items-center justify-between">
           <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-gray-300">Cloud Cover</span>
           </div>
           <span className="text-sm font-bold text-sky-400">{latestData.cloud_cover}%</span>
        </div>
