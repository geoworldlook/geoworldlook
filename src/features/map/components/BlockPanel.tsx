  ...
  return (
    <Card className="absolute top-4 right-4 w-80 md:w-96 shadow-2xl border-white/[0.1] bg-black/90 backdrop-blur-md animate-in slide-in-from-right duration-300 z-[1000]">
      ...
      <CardContent className="space-y-4">
        ...
        {/* Tab Switcher */}
        <div className="flex bg-white/[0.05] p-1 rounded-md">
           <button
             onClick={() => setActiveTab('ndvi')}
             className={`flex-1 text-[10px] py-1 rounded transition-colors ${activeTab === 'ndvi' ? 'bg-emerald-500 text-black font-bold' : 'text-gray-400'}`}
           >NDVI</button>
           <button
             onClick={() => setActiveTab('ndmi')}
             className={`flex-1 text-[10px] py-1 rounded transition-colors ${activeTab === 'ndmi' ? 'bg-sky-500 text-black font-bold' : 'text-gray-400'}`}
           >NDMI</button>
           <button
             onClick={() => setActiveTab('both')}
             className={`flex-1 text-[10px] py-1 rounded transition-colors ${activeTab === 'both' ? 'bg-white/20 text-white font-bold' : 'text-gray-400'}`}
           >BOTH</button>
        </div>
        ...
        <div className="flex items-center justify-between text-[10px] text-gray-500 italic">
          ...
          <div className="flex items-center gap-1">
             ...
             Avg Cloud: {latestData ? latestData.cloud_cover.toFixed(0) : '0'}%
          </div>
        </div>
      </CardContent>
    </Card>
