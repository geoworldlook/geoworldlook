    ...
    <Card className="absolute top-4 right-4 w-80 md:w-96 shadow-2xl border-white/[0.1] bg-black/90 backdrop-blur-md animate-in slide-in-from-right duration-300 z-[1000]">
      <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
        <div>
          ...
          <p className="text-xs text-gray-500">{block.area_ha ? `${block.area_ha} ha` : 'Area N/A'}</p>
        </div>
        ...
        {latestData ? (
          <>
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-white/[0.03] p-3 rounded-lg border border-white/[0.05]">
                <p className="text-[10px] text-gray-500 uppercase mb-1 flex items-center gap-1">
                  <TrendingUp className="w-3 h-3" /> Avg NDVI
                </p>
              ...
              <div className="bg-white/[0.03] p-3 rounded-lg border border-white/[0.05]">
                <p className="text-[10px] text-gray-500 uppercase mb-1 flex items-center gap-1">
                  <Droplets className="w-3 h-3" /> Avg NDMI
                </p>
            ...
            <div className="flex gap-2">
               <Button
                variant={activeMetric === 'all' ? 'secondary' : 'ghost'}
                size="sm"
              ...
              <Button
                variant={activeMetric === 'ndvi' ? 'secondary' : 'ghost'}
                size="sm"
                className="text-[10px] h-7 px-2"
              ...
              <Button
                variant={activeMetric === 'ndmi' ? 'secondary' : 'ghost'}
                size="sm"
                className="text-[10px] h-7 px-2"
          ...
          <div className="py-12 text-center">
            <p className="text-gray-500 text-xs italic">No historical data available for this block.</p>
          </div>
        )}
      </CardContent>
    </Card>
