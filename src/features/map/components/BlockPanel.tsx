...
export default function BlockPanel({ block, onClose }: BlockPanelProps) {
  const [indexType, setIndexType] = useState<'both' | 'ndvi' | 'ndmi'>('both');
          ...
          <>
            ...
            <div className="flex gap-2 mb-2">
              <Button
                ...
                className={`text-[10px] h-7 ${indexType === 'ndvi' ? 'bg-emerald-500/20 border-emerald-500' : ''}`}
              >
                NDVI
              </Button>
              <Button
                ...
                className={`text-[10px] h-7 ${indexType === 'ndmi' ? 'bg-sky-500/20 border-sky-500' : ''}`}
              >
                NDMI
              </Button>
              <Button
                variant="outline"
                size="sm"
                onClick={() => setIndexType('both')}
                className={`text-[10px] h-7 ${indexType === 'both' ? 'bg-white/10 border-white/20' : ''}`}
              >
                Both
              </Button>
            </div>
