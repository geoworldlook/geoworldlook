    ...
    <Card className="absolute top-4 right-4 w-80 md:w-96 shadow-2xl border-white/[0.1] bg-black/90 backdrop-blur-md animate-in slide-in-from-right duration-300 z-[1000]">
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div
            className={`cursor-pointer p-3 rounded-lg border transition-colors ${activeTab === 'ndvi' ? 'bg-emerald-500/10 border-emerald-500/50' : 'bg-white/[0.03] border-white/[0.05]'}`}
          ...
          <div
            className={`cursor-pointer p-3 rounded-lg border transition-colors ${activeTab === 'ndmi' ? 'bg-sky-500/10 border-sky-500/50' : 'bg-white/[0.03] border-white/[0.05]'}`}
        ...
        {latestData && (
          <div className="flex items-center justify-between text-[10px] text-gray-500">
            <div className="flex items-center gap-2">
              ...
              Cloud cover: {latestData.cloud_cover}%
            </div>
