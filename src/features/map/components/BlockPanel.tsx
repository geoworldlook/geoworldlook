...
export default function BlockPanel({ block, onClose }: BlockPanelProps) {
  ...
  // Filter to show distinct labels on X axis
    ...
    <Card className="absolute top-4 right-4 w-80 md:w-96 shadow-2xl border-white/[0.1] bg-black/90 backdrop-blur-md animate-in slide-in-from-right duration-300 z-[1000]">
      <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
        <div>
          ...
          <p className="text-xs text-gray-500">{block.area_ha} ha · Sentinel-2 Monitoring</p>
        </div>
        ...
        {latestData ? (
          <>
            <div className="grid grid-cols-2 gap-4">
              <div
                className="bg-white/[0.03] p-3 rounded-lg border border-white/[0.05] cursor-pointer hover:bg-white/5 transition-colors"
                onClick={() => setVisibleIndices(prev => ({ ...prev, ndvi: !prev.ndvi }))}
                style={{ opacity: visibleIndices.ndvi ? 1 : 0.4 }}
              >
              ...
              <div
                ...
                style={{ opacity: visibleIndices.ndmi ? 1 : 0.4 }}
              >
                <p className="text-[10px] text-gray-500 uppercase mb-1 flex items-center gap-1">
          ...
          <div className="py-10 text-center text-gray-500 text-sm">
            No satellite data available for this block yet.
          </div>
        )}
      </CardContent>
    </Card>
