...
interface BlockPanelProps {
  block: VineyardBlock & { stats: VineyardStat[] };
  onClose: () => void;
}
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex gap-2 mb-2">
          <Button
            variant="outline"
            size="sm"
            className={`text-[10px] h-7 ${activeMetric === 'ndvi' ? 'bg-emerald-500/20 border-emerald-500' : ''}`}
          ...
          <Button
            variant="outline"
            size="sm"
            className={`text-[10px] h-7 ${activeMetric === 'ndmi' ? 'bg-blue-500/20 border-blue-500' : ''}`}
          ...
          <Button
            variant="outline"
            size="sm"
            className={`text-[10px] h-7 ${activeMetric === 'both' ? 'bg-white/10 border-white/20' : ''}`}
        ...
        {latestData && (
          <div className="flex items-center gap-2 text-[10px] text-gray-500 italic">
            ...
            Last updated: {latestData.date} ({latestData.cloud_cover}% clouds)
          </div>
        )}
      </CardContent>
    </Card>
