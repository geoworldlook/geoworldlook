...
import { X, TrendingUp, Droplets, Calendar, Maximize2 } from 'lucide-react';
      ...
      <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
        <div>
          <p className="text-[10px] uppercase tracking-widest text-emerald-400 font-bold flex items-center gap-1">
            <Maximize2 className="w-3 h-3" /> Vineyard Block
          </p>
          <CardTitle className="text-lg text-white">{block.name}</CardTitle>
          <p className="text-xs text-gray-500">{block.area_ha} ha • Zielona Góra, PL</p>
        </div>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex gap-2">
           ...
           <Button
            ...
            className={`text-[10px] h-7 ${activeMetric === 'ndmi' ? 'bg-blue-500/20 border-blue-500/50' : ''}`}
           >NDMI</Button>
           <Button
            variant="outline"
            size="sm"
        ...
        <div className="flex items-center gap-2 text-[10px] text-gray-500 italic">
          ...
          Last satellite pass: {latestData?.date || 'N/A'}
        </div>
      </CardContent>
    </Card>
  );
