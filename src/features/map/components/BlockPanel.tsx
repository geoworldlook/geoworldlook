    ...
    <Card className="absolute top-4 right-4 w-80 md:w-96 shadow-2xl border-white/[0.1] bg-black/90 backdrop-blur-md animate-in slide-in-from-right duration-300 z-[1000]">
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div
            className={`cursor-pointer transition-all p-3 rounded-lg border ${activeTab === 'NDVI' ? 'bg-emerald-500/10 border-emerald-500/50' : 'bg-white/[0.03] border-white/[0.05]'}`}
          ...
          <div
            className={`cursor-pointer transition-all p-3 rounded-lg border ${activeTab === 'NDMI' ? 'bg-sky-500/10 border-sky-500/50' : 'bg-white/[0.03] border-white/[0.05]'}`}
        ...
        <div className="bg-white/[0.03] p-2 rounded-lg border border-white/[0.05] flex items-center justify-between">
           <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] text-gray-400 uppercase">Cloud Cover</span>
           </div>
           <span className="text-xs font-medium text-white">{latestData.cloud_cover}%</span>
        </div>
