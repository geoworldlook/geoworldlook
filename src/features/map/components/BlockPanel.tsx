    ...
    <Card className="absolute top-4 right-4 w-80 md:w-96 shadow-2xl border-white/[0.1] bg-black/90 backdrop-blur-md animate-in slide-in-from-right duration-300 z-[1000]">
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div
            className={`p-3 rounded-lg border cursor-pointer transition-all ${activeMetric === 'ndvi' ? 'bg-emerald-500/10 border-emerald-500/50' : 'bg-white/[0.03] border-white/[0.05]'}`}
          ...
          <div
            className={`p-3 rounded-lg border cursor-pointer transition-all ${activeMetric === 'ndmi' ? 'bg-sky-500/10 border-sky-500/50' : 'bg-white/[0.03] border-white/[0.05]'}`}
        ...
        <div className="flex flex-col gap-2">
            <div className="flex items-center gap-2 text-[10px] text-gray-500">
                ...
                Cloud Cover: {latestData ? latestData.cloud_cover.toFixed(1) : 'N/A'}%
            </div>
            <div className="flex items-center gap-2 text-[10px] text-gray-500 italic">
                <Calendar className="w-3 h-3" />
                Last analysis: {latestData ? latestData.date : 'N/A'}
            </div>
        </div>
      </CardContent>
    </Card>
