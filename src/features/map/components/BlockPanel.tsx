...
export default function BlockPanel({ block, onClose }: BlockPanelProps) {
  ...
  const [activeMetric, setActiveMetric] = useState<'all' | 'ndvi' | 'ndmi'>('all');
  ...
  return (
    <Card className="absolute top-4 right-4 w-80 md:w-96 shadow-2xl border-white/[0.1] bg-black/90 backdrop-blur-md animate-in slide-in-from-right duration-300 z-[1000]">
      <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
        <div>
          <p className="text-[10px] uppercase tracking-widest text-emerald-400 font-bold">Vineyard Block Analysis</p>
          <CardTitle className="text-lg text-white">{block.name}</CardTitle>
          <p className="text-xs text-gray-500">{block.area_ha} ha • Sentinel-2 L2A</p>
        </div>
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <button
            onClick={() => setActiveMetric(activeMetric === 'ndvi' ? 'all' : 'ndvi')}
            className={`text-left p-3 rounded-lg border transition-all ${activeMetric === 'ndvi' ? 'bg-emerald-500/10 border-emerald-500/50' : 'bg-white/[0.03] border-white/[0.05]'}`}
          >
          ...
          <button
            onClick={() => setActiveMetric(activeMetric === 'ndmi' ? 'all' : 'ndmi')}
            className={`text-left p-3 rounded-lg border transition-all ${activeMetric === 'ndmi' ? 'bg-sky-500/10 border-sky-500/50' : 'bg-white/[0.03] border-white/[0.05]'}`}
          >
            <p className="text-[10px] text-gray-500 uppercase mb-1 flex items-center gap-1">
        ...
        <div className="h-56 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData}>
              ...
              {(activeMetric === 'all' || activeMetric === 'ndmi') && (
                <Line
                  name="NDMI"
                  type="monotone"
                  dataKey="ndmi_mean"
        ...
        <div className="flex items-center justify-between text-[10px] text-gray-500">
          ...
          <div className="flex items-center gap-1">
            <Cloud className="w-3 h-3" /> {latestData?.cloud_cover || 0}%
          </div>
        </div>
      </CardContent>
    </Card>
