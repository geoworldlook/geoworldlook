...
export default function BlockPanel({ block, onClose }: BlockPanelProps) {
  ...
  return (
    <Card
      id="block-panel"
      className="absolute top-4 right-4 w-80 md:w-96 shadow-2xl border-white/[0.1] bg-black/90 backdrop-blur-md animate-in slide-in-from-right duration-300 z-[1000]"
    >
      ...
      <CardContent className="space-y-4">
        ...
        <div className="bg-white/[0.03] p-2 rounded-lg border border-white/[0.05] flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Cloud className="w-3 h-3 text-sky-400" />
            <span className="text-[10px] text-gray-500 uppercase">Cloud Cover</span>
          </div>
          <span className="text-xs font-bold text-sky-400">{latestData.cloud_cover}%</span>
        </div>
        ...
        <div className="h-48 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData}>
              ...
              />
              <YAxis
                domain={activeMetric === 'ndvi' ? [0, 1] : [-1, 1]}
                stroke="#666"
                fontSize={10}
                tickLine={false}
                axisLine={false}
              ...
              <Tooltip
                ...
                itemStyle={{ color: activeMetric === 'ndvi' ? '#10b981' : '#3b82f6' }}
                labelFormatter={(label) => `Date: ${label}`}
              />
              <Line
                type="monotone"
                dataKey={activeMetric === 'ndvi' ? 'ndvi_mean' : 'ndmi_mean'}
                stroke={activeMetric === 'ndvi' ? '#10b981' : '#3b82f6'}
                strokeWidth={2}
                dot={false}
                activeDot={{ r: 4, fill: activeMetric === 'ndvi' ? '#10b981' : '#3b82f6' }}
              />
            </LineChart>
          </ResponsiveContainer>
        </div>
