...
export default function BlockPanel({ block, onClose }: BlockPanelProps) {
  const [activeSeries, setActiveSeries] = useState<'both' | 'ndvi' | 'ndmi'>('both');
    ...
    <Card className="absolute top-4 right-4 w-80 md:w-96 shadow-2xl border-white/[0.1] bg-black/90 backdrop-blur-md animate-in slide-in-from-right duration-300 z-[1000]">
      <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
        <div>
          ...
          <p className="text-xs text-gray-500">{block.area_ha} ha · Polygons</p>
        </div>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex gap-2 mb-2">
            <Button
                variant="outline"
                size="sm"
                className={`text-[10px] h-7 px-2 ${activeSeries === 'ndvi' ? 'bg-emerald-500/20 border-emerald-500/50' : ''}`}
                onClick={() => setActiveSeries('ndvi')}
            >
                NDVI Only
            </Button>
            <Button
                variant="outline"
                size="sm"
                className={`text-[10px] h-7 px-2 ${activeSeries === 'ndmi' ? 'bg-blue-500/20 border-blue-500/50' : ''}`}
                onClick={() => setActiveSeries('ndmi')}
            >
                NDMI Only
            </Button>
            <Button
                variant="outline"
                size="sm"
                className={`text-[10px] h-7 px-2 ${activeSeries === 'both' ? 'bg-white/10 border-white/20' : ''}`}
        ...
        <div className="h-48 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData}>
              ...
              {(activeSeries === 'ndvi' || activeSeries === 'both') && (
                <Line
                    type="monotone"
                    dataKey="ndvi_mean"
                    name="NDVI (Veg)"
              ...
              {(activeSeries === 'ndmi' || activeSeries === 'both') && (
                <Line
                    type="monotone"
                    dataKey="ndmi_mean"
                    name="NDMI (Moist)"
