...
export default function BlockPanel({ block, onClose }: BlockPanelProps) {
  const [view, setView] = useState<'ndvi' | 'ndmi' | 'both'>('both');
    ...
    <Card className="absolute top-4 right-4 w-80 md:w-96 shadow-2xl border-white/[0.1] bg-black/90 backdrop-blur-md animate-in slide-in-from-right duration-300 z-[1000]">
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div
            className={`p-3 rounded-lg border cursor-pointer transition-colors ${view === 'ndvi' ? 'bg-emerald-500/10 border-emerald-500/50' : 'bg-white/[0.03] border-white/[0.05]'}`}
            onClick={() => setView(view === 'ndvi' ? 'both' : 'ndvi')}
          >
          ...
          <div
            className={`p-3 rounded-lg border cursor-pointer transition-colors ${view === 'ndmi' ? 'bg-sky-500/10 border-sky-500/50' : 'bg-white/[0.03] border-white/[0.05]'}`}
            onClick={() => setView(view === 'ndmi' ? 'both' : 'ndmi')}
          >
            <p className="text-[10px] text-gray-500 uppercase mb-1 flex items-center gap-1">
        ...
        <div className="flex items-center justify-between text-[10px] text-gray-500 italic">
          <div className="flex items-center gap-2">
            ...
            Avg Cloud: {latestData?.cloud_cover}%
          </div>
          <div className="flex items-center gap-2">
