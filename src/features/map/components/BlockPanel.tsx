...
export default function BlockPanel({ block, onClose }: BlockPanelProps) {
  const [indexType, setIndexType] = useState<'ndvi' | 'ndmi'>('ndvi');
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <button
            ...
            className={`p-3 rounded-lg border transition-all text-left ${indexType === 'ndvi' ? 'bg-emerald-500/10 border-emerald-500/50' : 'bg-white/[0.03] border-white/[0.05]'}`}
          >
            <p className="text-[10px] text-gray-500 uppercase mb-1 flex items-center gap-1">
              <TrendingUp className="w-3 h-3" /> NDVI (Mean)
            </p>
            <p className={`text-2xl font-bold ${indexType === 'ndvi' ? 'text-emerald-400' : 'text-gray-400'}`}>
              {latestData.ndvi_mean.toFixed(2)}
            </p>
          </button>
          <button
            onClick={() => setIndexType('ndmi')}
            className={`p-3 rounded-lg border transition-all text-left ${indexType === 'ndmi' ? 'bg-sky-500/10 border-sky-500/50' : 'bg-white/[0.03] border-white/[0.05]'}`}
          >
            <p className="text-[10px] text-gray-500 uppercase mb-1 flex items-center gap-1">
              <Droplets className="w-3 h-3" /> NDMI (Water)
            </p>
            <p className={`text-2xl font-bold ${indexType === 'ndmi' ? 'text-sky-400' : 'text-gray-400'}`}>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData}>
              ...
              <Tooltip 
                ...
                itemStyle={{ color: indexType === 'ndvi' ? '#10b981' : '#0ea5e9' }}
                labelFormatter={(label) => `Date: ${label}`}
              />
              <Line 
                type="monotone" 
                dataKey={indexType === 'ndvi' ? 'ndvi_mean' : 'ndmi_mean'}
                stroke={indexType === 'ndvi' ? '#10b981' : '#0ea5e9'}
                strokeWidth={2} 
                dot={false}
                activeDot={{ r: 4, fill: indexType === 'ndvi' ? '#10b981' : '#0ea5e9' }}
              />
            </LineChart>
          </ResponsiveContainer>
        </div>
