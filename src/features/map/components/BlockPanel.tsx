    ...
    <Card className="absolute top-4 right-4 w-80 md:w-96 shadow-2xl border-white/[0.1] bg-black/90 backdrop-blur-md animate-in slide-in-from-right duration-300 z-[1000]">
      <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
        <div>
          ...
          <CardTitle className="text-lg text-white">{block.name}</CardTitle>
          <p className="text-xs text-gray-500">{block.area_ha ? `${block.area_ha} ha` : 'Area unknown'}</p>
        </div>
        <Button variant="ghost" size="icon" onClick={onClose} className="text-gray-400 hover:text-white">
          <X className="w-4 h-4" />
        </Button>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex gap-2 mb-2">
          <Button
            variant="outline"
            size="sm"
            className={`text-[10px] h-7 ${visibleIndex === 'ndvi' ? 'bg-emerald-500/20 border-emerald-500' : 'bg-transparent'}`}
          ...
          <Button
            variant="outline"
            size="sm"
            className={`text-[10px] h-7 ${visibleIndex === 'ndmi' ? 'bg-sky-500/20 border-sky-500' : 'bg-transparent'}`}
          ...
          <Button
            variant="outline"
            size="sm"
            className={`text-[10px] h-7 ${visibleIndex === 'both' ? 'bg-white/10 border-white/20' : 'bg-transparent'}`}
        ...
        <div className="h-48 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData}>
              ...
              {(visibleIndex === 'ndvi' || visibleIndex === 'both') && (
                <Line
                  name="NDVI"
                  type="monotone"
                  dataKey="ndvi_mean"
              ...
              {(visibleIndex === 'ndmi' || visibleIndex === 'both') && (
                <Line
                  name="NDMI"
                  type="monotone"
                  dataKey="ndmi_mean"
        ...
        <div className="flex items-center justify-between text-[10px] text-gray-500">
          ...
          <div className="flex items-center gap-1">
            <Cloud className="w-3 h-3" /> {latestData.cloud_cover}%
          </div>
        </div>
      </CardContent>
    </Card>
