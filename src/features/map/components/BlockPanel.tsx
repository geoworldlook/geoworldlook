  ...
  if (!latestData) {
      return (
        <Card className="absolute top-4 right-4 w-80 md:w-96 shadow-2xl border-white/[0.1] bg-black/90 backdrop-blur-md z-[1000]">
            ...
            <CardContent>
                <p className="text-gray-500 text-sm italic">No satellite data available for this block.</p>
            </CardContent>
        </Card>
      );
  }
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-3 gap-2">
          <button
            ...
            className={`p-2 rounded-lg border transition-all ${activeMetric === 'ndvi' ? 'bg-emerald-500/20 border-emerald-500/50' : 'bg-white/[0.03] border-white/[0.05]'}`}
          >
          ...
          <button
            ...
            className={`p-2 rounded-lg border transition-all ${activeMetric === 'ndmi' ? 'bg-blue-500/20 border-blue-500/50' : 'bg-white/[0.03] border-white/[0.05]'}`}
          >
            <p className="text-[8px] text-gray-500 uppercase mb-1 flex items-center gap-1">
