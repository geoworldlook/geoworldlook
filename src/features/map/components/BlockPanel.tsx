...
export default function BlockPanel({ block, stats, onClose }: BlockPanelProps) {
  ...
  // Filter to show unique month/year combinations on X axis
  const tickIndices = chartData
    .map((d, i) => ({ monthYear: d.monthYear, index: i }))
    .filter((v, i, a) => a.findIndex(t => t.monthYear === v.monthYear) === i)
    .map(v => v.index);

  const monthTicks = tickIndices.map(i => chartData[i].dateFormatted);
    ...
    <Card className="absolute top-4 right-4 w-80 md:w-96 shadow-2xl border-white/[0.1] bg-black/90 backdrop-blur-md animate-in slide-in-from-right duration-300 z-[1000]">
      <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
        <div>
          ...
          <p className="text-xs text-gray-500">{block.area_ha} ha · Polygon Analysis</p>
        </div>
      ...
      <CardContent className="space-y-4">
        {latestData ? (
          <>
            ...
            <div className="flex gap-2 mb-2">
               <Button
                variant="outline"
                size="sm"
                className={`text-[10px] h-7 ${activeTab === 'ndvi' ? 'bg-emerald-500/20 border-emerald-500/50' : 'bg-transparent'}`}
                onClick={() => setActiveTab('ndvi')}
               >NDVI</Button>
               <Button
                variant="outline"
                size="sm"
                className={`text-[10px] h-7 ${activeTab === 'ndmi' ? 'bg-sky-500/20 border-sky-500/50' : 'bg-transparent'}`}
                onClick={() => setActiveTab('ndmi')}
               >NDMI</Button>
               <Button
                variant="outline"
                size="sm"
                className={`text-[10px] h-7 ${activeTab === 'both' ? 'bg-white/10 border-white/20' : 'bg-transparent'}`}
          ...
          <div className="h-48 flex items-center justify-center text-gray-500 text-sm italic">
            No historical satellite data available
          </div>
        )}
      </CardContent>
    </Card>
