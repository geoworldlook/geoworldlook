    ...
    <Card className="absolute top-4 right-4 w-80 md:w-96 shadow-2xl border-white/[0.1] bg-black/90 backdrop-blur-md animate-in slide-in-from-right duration-300 z-[1000]">
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div className={`p-3 rounded-lg border cursor-pointer transition-colors ${activeMetric === 'ndvi' ? 'bg-emerald-500/10 border-emerald-500/50' : 'bg-white/[0.03] border-white/[0.05]'}`}
          ...
          <div className={`p-3 rounded-lg border cursor-pointer transition-colors ${activeMetric === 'ndmi' ? 'bg-sky-500/10 border-sky-500/50' : 'bg-white/[0.03] border-white/[0.05]'}`}
        ...
        <div className="flex items-center justify-between text-[10px] text-gray-500">
          <div className="flex items-center gap-2 italic">
            ...
            Last updated: {latestData ? latestData.date : '--'}
          </div>
          <div className="flex items-center gap-1">
            <Cloud className="w-3 h-3" /> {latestData ? latestData.cloud_cover : 0}%
          </div>
        </div>
      </CardContent>
    </Card>
