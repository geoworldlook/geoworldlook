...
interface StationPanelProps {
  block: VineyardBlockData;
  onClose: () => void;
}

export default function StationPanel({ block, onClose }: StationPanelProps) {
  ...
  return (
    <Card className="absolute top-4 right-4 w-80 md:w-96 shadow-2xl border-white/[0.1] bg-black/90 backdrop-blur-md animate-in slide-in-from-right duration-300 z-[1000]">
      <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
        <div>
          <p className="text-[10px] uppercase tracking-widest text-emerald-400 font-bold">Działka Winnicy / Vineyard Block</p>
          <CardTitle className="text-lg text-white">{block.name}</CardTitle>
          <p className="text-xs text-gray-500 flex items-center gap-1 mt-0.5">
            <Maximize2 className="w-3 h-3 text-emerald-400" /> Powierzchnia: {block.area_ha} ha
          </p>
        </div>
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-3 gap-2">
          ...
          <div className="bg-white/[0.03] p-2.5 rounded-lg border border-white/[0.05]">
            <p className="text-[9px] text-gray-400 uppercase mb-1 flex items-center gap-1">
              <Droplets className="w-3 h-3 text-blue-400" /> NDMI
            </p>
          ...
          <div className="bg-white/[0.03] p-2.5 rounded-lg border border-white/[0.05]">
            <p className="text-[9px] text-gray-400 uppercase mb-1 flex items-center gap-1">
              <Cloud className="w-3 h-3 text-sky-400" /> Chmury
            </p>
