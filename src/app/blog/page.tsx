  ...
  return (
    <div className="container mx-auto px-4 py-24 space-y-16">
      ...
      <div className="space-y-8">
        {blogPosts.map((post) => (
          <Link key={post.slug} href={`/blog/${post.slug}`} className="block">
            <Card className="group border-border/50 bg-card/50 hover:bg-card hover:border-primary/30 transition-all p-4 md:p-8">
              <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 items-center">
                ...
                <div className="lg:col-span-3 space-y-4">
                  <div className="flex flex-wrap gap-4 items-center text-xs text-muted-foreground">
                    <span className="flex items-center gap-1 font-mono uppercase tracking-widest bg-muted px-2 py-0.5 rounded text-foreground">{post.date}</span>
                    <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> {post.readTime}</span>
                    <div className="flex gap-2">
                      {post.tags.map(tag => (
                        <span key={tag} className="flex items-center gap-1 text-primary"><Tag className="w-3 h-3" /> {tag}</span>
                      ))}
                    </div>
                  </div>
                  <CardTitle className="text-2xl md:text-3xl font-headline group-hover:text-primary transition-colors">{post.title}</CardTitle>
                  <CardDescription className="text-base leading-relaxed md:line-clamp-2">
                    {post.excerpt}
                  </CardDescription>
                  <div className="pt-2">
      ...
      <div className="flex justify-center pt-12">
        <Button variant="outline" className="px-12 py-6 rounded-full text-lg">Load More Articles</Button>
      </div>
    </div>
  )
}
