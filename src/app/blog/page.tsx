          ...
          <Link key={post.slug} href={`/blog/${post.slug}`} className="block">
            <Card className="group border-white/[0.06] bg-[#111]/50 hover:bg-[#111] hover:border-emerald-400/30 transition-all overflow-hidden p-0">
              <div className="flex flex-col md:flex-row">
                <div className="md:w-1/3 relative h-64 md:h-auto overflow-hidden grayscale group-hover:grayscale-0 transition-all">
                  <Image 
                ...
                <div className="md:w-2/3 p-6 md:p-8 space-y-4">
                  <div className="flex flex-wrap gap-4 items-center text-xs text-gray-500">
                    <span className="bg-emerald-400/10 text-emerald-400 px-2 py-0.5 rounded">{post.date}</span>
                    <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> {post.readTime}</span>
                  </div>
                  <CardTitle className="text-2xl font-bold text-white group-hover:text-emerald-400 transition-colors">{post.title}</CardTitle>
                  <CardDescription className="text-base leading-relaxed text-gray-400 line-clamp-2">{post.excerpt}</CardDescription>
                  <div className="pt-2">
