    ...
    <div className="container mx-auto px-4 py-24 space-y-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {analyses.map((analysis) => {
          ...
          return (
            <Card key={analysis.id} className="group overflow-hidden bg-[#111111] border-white/[0.06] hover:border-emerald-400/50 transition-all">
              <div className="relative h-64 overflow-hidden">
                ...
                <div className="absolute top-4 right-4">
                  <Badge className="bg-black/60 backdrop-blur-md text-emerald-400 border-emerald-400/20">
                    {analysis.category}
                  </Badge>
                </div>
              </div>
              <CardHeader>
                ...
                <CardTitle className="text-xl group-hover:text-emerald-400 transition-colors">{analysis.title}</CardTitle>
                <CardDescription className="line-clamp-3 leading-relaxed text-gray-400">
                  {analysis.description}
                </CardDescription>
              </CardHeader>
              <CardFooter className="pt-0">
                <Link href={`/analyses/${analysis.id}`} className="w-full">
                  <div className="flex items-center justify-center gap-2 text-sm font-bold text-emerald-400 transition-all py-2 border border-emerald-400/20 rounded-lg group-hover:bg-emerald-400/10">
