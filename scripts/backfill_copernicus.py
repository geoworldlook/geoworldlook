...
def fetch_block_data(token: str, block: dict, days_back: int = 365) -> list:
    # Use the polygon geometry from the database
    geometry = block['geom']
    ...
    print(f"📡 Pobieranie 365 dni telemetrii dla działki: {block['name']} ({block['area_ha']} ha)...")
...
def main():
    ...
    # Pobieramy działki winnicy z Supabase, używając st_asgeojson dla pola geom
    # Supabase (PostgREST) nie obsługuje bezpośrednio st_asgeojson w selekcie przez SDK bez RPC
    # ale możemy użyć raw query lub post-processingu jeśli mamy postgis.
    # W tym skrypcie użyjemy RPC jeśli dostępne, lub spróbujemy sparsować geom jeśli jest stringiem JSON.
    
    try:
        # Próba pobrania przez select - jeśli Supabase zwraca GeoJSON string/object
        # W PostgREST można czasami użyć modyfikatorów, ale tutaj spróbujemy najprostszego podejścia:
        blocks = supabase.table("vineyard_blocks").select("id, name, area_ha, geom").execute().data
    ...
    # Post-processing: upewnij się, że geom to obiekt JSON
    import json
    for block in blocks:
        if isinstance(block['geom'], str):
            try:
                block['geom'] = json.loads(block['geom'])
            except:
                # Jeśli to EWKB, będziemy mieli problem bez dodatkowej biblioteki.
                # Zakładamy że user skonfigurował Supabase by zwracało JSON lub używamy st_asgeojson.
                pass
