...
def fetch_block_data(token: str, block: dict, days_back: int = 365) -> list:
    # Use geometry from GeoJSON properties or geom field
    # In our RPC we return GeoJSON, but here we might be fetching from table directly or using the RPC output.
    # Let's assume we use the geometry from the block dict.
    ...
    print(f"📡 Pobieranie telemetrii dla działki: {block['name']} ({block['area_ha']} ha)...")
...
def main():
    ...
    # Pobieramy działki. Uwaga: Supabase/PostgREST może zwrócić geometrię jako WKB (hex string).
    # Aby mieć GeoJSON, lepiej użyć naszej funkcji RPC lub st_asgeojson w select (ale PostgREST tego bezpośrednio nie wspiera łatwo).
    # Wykorzystamy RPC get_vineyard_blocks_geojson i wyciągniemy z niego features.
    
    try:
        rpc_resp = supabase.rpc("get_vineyard_blocks_geojson").execute()
        blocks_geojson = rpc_resp.data
        if not blocks_geojson or 'features' not in blocks_geojson:
