...
# Evalscript obliczający NDVI (roślinność) i NDMI (wilgotność/stres wodny)
INDICES_EVALSCRIPT = """
//VERSION=3
function setup() {
  return {
...
def fetch_block_data(token: str, block: dict, days_back: int = 365) -> list:
    # Używamy geometrii poligonu przekazanej z bazy (GeoJSON)
    # CDSE Statistical API przyjmuje geometrię w formacie GeoJSON
    ...
    payload = {
        "input": {
            "bounds": {
                "geometry": block['geom'], # 'geom' musi być w formacie GeoJSON (obsługiwane przez RPC w Supabase)
    ...
    print(f"📡 Pobieranie {days_back} dni telemetrii dla poligonu: {block['name']}...")
...
def main():
    print("🚀 Start procesu Backfill dla Poligonów (NDVI + NDMI)...")
    
    # Pobieramy działki z bazy z geometrią jako GeoJSON
    # UWAGA: Wymaga to albo RPC, albo CAST w select, ale tutaj najprościej użyć PostGIS ST_AsGeoJSON
    # Wykonujemy bezpośrednie zapytanie SQL przez rpc jeśli mamy taką funkcję lub mapujemy ręcznie
    # Dla uproszczenia skryptu zakładamy, że pobieramy dane z vineyard_blocks i rzutujemy geom

    try:
        # Pobieramy ID i geometrie (jako GeoJSON)
        # W Supabase Python Client najlepiej użyć rpc jeśli zdefiniowaliśmy funkcję pomocniczą
        # Alternatywnie: SELECT id, name, ST_AsGeoJSON(geom)::json as geom FROM vineyard_blocks

        blocks_resp = supabase.rpc("get_vineyard_blocks_geojson").execute()
        if not blocks_resp.data or 'features' not in blocks_resp.data:
            print("❌ Brak działek w bazie Supabase lub błąd GeoJSON.")
            return

        blocks = []
        for feature in blocks_resp.data['features']:
            blocks.append({
    ...
    if records_to_upsert:
        print(f"💾 Wysyłanie {len(records_to_upsert)} rekordów do vineyard_stats...")
        try:
            # Upsert do nowej tabeli
