...
# Evalscript dla NDVI, NDMI i Chmur
...
def fetch_block_data(token: str, block: dict, days_back: int = 365) -> list:
    ...
    print(f"📡 Pobieranie {days_back} dni danych dla działki: {block['properties']['name']}...")
...
def main():
    ...
    # Pobieramy bloki za pomocą funkcji RPC
    try:
    ...
    if not blocks_collection or not blocks_collection.get('features'):
    ...
    for block in blocks_collection['features']:
    ...
    if records_to_upsert:
        ...
        try:
            # Dzielenie na paczki po 100, aby uniknąć problemów z wielkością requestu
