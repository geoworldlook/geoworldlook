...
def fetch_block_data(token: str, block: dict, days_back: int = 365) -> list:
    ...
    print(f"📡 Pobieranie 365 dni telemetrii dla parceli: {block['name']}...")
...
def main():
    ...
    # Pobieramy parcele z Supabase (geom pobieramy jako GeoJSON przez ST_AsGeoJSON w widoku lub tutaj)
    # Dla uproszczenia zakładamy, że geom jest dostępny (jeśli używamy RPC to jest OK)
    # W pythonie bez RPC możemy spróbować pobrać bezpośrednio
    blocks = supabase.table("vineyard_blocks").select("id, name, geom").execute().data
    if not blocks:
        print("❌ Brak parcel w bazie Supabase.")
        return
