...
# Evalscript obliczający NDVI oraz NDMI (B08 i B11)
EVALSCRIPT = """
//VERSION=3
function setup() {
  return {
...
def fetch_block_data(token: str, feature: dict, days_back: int = 365) -> list:
    # Używamy geometrii bezpośrednio z GeoJSON (Polygon)
    ...
    print(f"📡 Pobieranie 365 dni telemetrii dla działki: {feature['properties']['name']}...")
    
    resp = requests.post(STATS_API_URL, json=payload, headers=headers)
    if resp.status_code != 200:
        print(f"⚠️ Błąd API CDSE dla {feature['properties']['name']}: {resp.text}")
        return []
...
def main():
    ...
    # Pobieramy bloki winnicy jako GeoJSON przez RPC
    try:
