...
def fetch_block_data(token: str, block: dict, days_back: int = 365) -> list:
    # Używamy geometrii poligonu z bazy
    # Zakładamy, że geom jest pobierany jako GeoJSON object
    geometry = block['geom']
...
def main():
    ...
    # Uwaga: Supabase/PostgREST może zwracać geometrię jako WKB (hex).
    # W produkcyjnym rozwiązaniu lepiej użyć RPC lub rzutowania w select.
    # Tutaj użyjemy RPC jeśli zdefiniowaliśmy, lub spróbujemy pobrać normalnie.
    
    # Próba pobrania przez RPC aby mieć GeoJSON od razu (używamy stworzonego wcześniej RPC)
    # get_vineyard_blocks_geojson zwraca FeatureCollection
    rpc_resp = supabase.rpc("get_vineyard_blocks_geojson").execute()
    if not rpc_resp.data or 'features' not in rpc_resp.data:
        print("❌ Brak bloków w bazie Supabase lub błąd RPC.")
        return

    blocks = []
    for f in rpc_resp.data['features']:
        blocks.append({
            'id': f['properties']['id'],
