...
def main():
    ...
    # Pobieramy działki z Supabase używając RPC (dla GeoJSON)
    # Wymaga wgrania funkcji get_vineyard_blocks_geojson() z migracji SQL.
    try:
        response = supabase.rpc("get_vineyard_blocks_geojson").execute()
        blocks = response.data
    except Exception as e:
        print(f"❌ Krytyczny błąd: Nie udało się pobrać geometrii działek przez RPC: {e}")
        print("Upewnij się, że migracja SQL z funkcją 'get_vineyard_blocks_geojson' została wykonana.")
        return
