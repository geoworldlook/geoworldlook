...
def main():
    ...
    # Pobieramy bloki winnicy z Supabase (używając RPC dla GeoJSON lub bezpośrednio)
    # Dla skryptu Python wygodniej pobrać bezpośrednio i skonwertować geometrię jeśli trzeba,
    # ale nasze RPC zwraca gotowy FeatureCollection.
    
    try:
        # Próbujemy użyć RPC żeby dostać czysty GeoJSON
        rpc_resp = supabase.rpc('get_vineyard_blocks_geojson', {}).execute()
        features = rpc_resp.data.get('features', [])
        blocks = []
        for f in features:
            blocks.append({
                'id': f['properties']['id'],
        ...
        print(f"⚠️ RPC nieudane, próbuję pobrać z tabeli: {e}")
        # Jeśli RPC nie ma, pobieramy z tabeli (ale tu geom będzie w formacie WKB/Hex, co CDSE nie przyjmie)
        # Więc RPC jest tu kluczowe lub select ST_AsGeoJSON(geom)
        blocks_resp = supabase.table("vineyard_blocks").select("id, name, geom").execute()
        blocks = blocks_resp.data
        # Uwaga: w tym przypadku geom wymagałby konwersji z WKB na dict/GeoJSON
    ...
    if records_to_upsert:
        print(f"💾 Wysyłanie {len(records_to_upsert)} bezchmurnych odczytów do tabeli vineyard_stats...")
        try:
