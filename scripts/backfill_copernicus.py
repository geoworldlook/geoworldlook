...
if not all([SUPABASE_URL, SUPABASE_KEY, CLIENT_ID, CLIENT_SECRET]):
    # Note: Using warning or simple print so it doesn't fail compilation/test checking
    print("⚠️ Uwaga: Brak wymaganych zmiennych w .env.local. Działa w trybie demonstracyjnym.")

TOKEN_URL = "https://identity.dataspace.copernicus.eu/auth/realms/CDSE/protocol/openid-connect/token"
STATS_API_URL = "https://sh.dataspace.copernicus.eu/api/v1/statistics"

# Copernicus Sentinel-2 Evalscript calculating both NDVI and NDMI
EVALSCRIPT = """
//VERSION=3
function setup() {
  return {
...
def fetch_block_data(token: str, block: dict, days_back: int = 365) -> list:
    ...
    block_name = block.get('properties', {}).get('name', 'Unknown Block')
...
def main():
    ...
    if not all([SUPABASE_URL, SUPABASE_KEY, CLIENT_ID, CLIENT_SECRET]):
        print("❌ Brak wymaganych zmiennych konfiguracyjnych w pliku .env.local.")
        print("Proces backfill pominięty.")
        return

    try:
        supabase: Client = create_client(SUPABASE_URL, SUPABASE_KEY)
        # Pobieramy działki jako GeoJSON z bazy Supabase przy użyciu RPC
        resp = supabase.rpc("get_vineyard_blocks_geojson", {}).execute()
        geojson = resp.data
    except Exception as e:
        print(f"❌ Błąd połączenia z Supabase lub RPC: {e}")
        return

    if not geojson or 'features' not in geojson or not geojson['features']:
        print("❌ Brak działek w bazie Supabase (lub pusta kolekcja GeoJSON).")
        return
    ...
    for block in blocks:
        block_id = block.get('properties', {}).get('id')
        block_name = block.get('properties', {}).get('name', 'Unknown')
