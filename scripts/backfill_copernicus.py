...
function evaluatePixel(sample) {
  let ndvi_val = (sample.B08 + sample.B04 !== 0) ? (sample.B08 - sample.B04) / (sample.B08 + sample.B04) : 0;
  let ndmi_val = (sample.B08 + sample.B11 !== 0) ? (sample.B08 - sample.B11) / (sample.B08 + sample.B11) : 0;
...
def fetch_block_data(token: str, block: dict, days_back: int = 365) -> list:
    # Geometry extracted from block['geom']
    geom = block['geom']
