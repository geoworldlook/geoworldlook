...
# Załadowanie zmiennych z pliku .env.local lub systemowych
...
if not all([SUPABASE_URL, SUPABASE_KEY, CLIENT_ID, CLIENT_SECRET]):
    raise ValueError("Krytyczny błąd: Brak wymaganych zmiennych w środowisku (SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, COPERNICUS_CLIENT_ID, COPERNICUS_CLIENT_SECRET)")
...
# Evalscript obliczający NDVI, NDMI, cloud cover i valid data mask
EVALSCRIPT = """
//VERSION=3
function setup() {
  return {
...
def fetch_block_data(token: str, geometry: dict, block_name: str, days_back: int = 365) -> list:
    ...
    print(f"📡 Pobieranie {days_back} dni telemetrii satelitarnej dla działki: {block_name}...")
...
def main():
    ...
    # Pobieramy geometrię działek wyłącznie przez RPC get_vineyard_blocks_geojson
    print("🛰️ Pobieranie obiektów z bazy przez RPC get_vineyard_blocks_geojson...")
    rpc_response = supabase.rpc("get_vineyard_blocks_geojson", {}).execute()
    geojson_collection = rpc_response.data

    if not geojson_collection or "features" not in geojson_collection:
        print("❌ Brak poprawnych danych GeoJSON z bazy Supabase.")
        return

    features = geojson_collection.get("features", [])
    if not features:
        print("❌ Brak zdefiniowanych działek w bazie.")
        return

    print(f" znaleziono {len(features)} działek. Rozpoczynanie autoryzacji...")
    ...
    for feature in features:
        ...
        block_name = props.get("name", "Nieznana działka")
        geometry = feature.get("geometry")

        if not block_id or not geometry:
            print(f"⚠️ Pomiń niepoprawny obiekt: {props}")
            continue
    ...
    if records_to_upsert:
        print(f"💾 Wysyłanie {len(records_to_upsert)} bezchmurnych odczytów do bazy Supabase...")
        try:
            supabase.table("vineyard_stats").upsert(records_to_upsert, on_conflict="block_id, date").execute()
            print("✅ Backfill zakończony sukcesem dla wszystkich działek!")
        except Exception as e:
            print(f"❌ Błąd zapisu: {e}")
    else:
        print("⚠️ Brak bezchmurnych danych do zapisu.")
