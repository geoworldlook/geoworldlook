...
# Evalscript obliczający NDVI i NDMI oraz zachmurzenie
EVALSCRIPT = """
//VERSION=3
function setup() {
  return {
...
def fetch_block_data(token: str, block: dict, days_back: int = 365) -> list:
    # Używamy geometrii poligonu z bazy danych
    geom = block.get('geom')
    
    # Obsługa różnych formatów (GeoJSON jako słownik lub string)
    if isinstance(geom, str):
        try:
            geom = json.loads(geom)
        except json.JSONDecodeError:
            print(f"⚠️ Nieprawidłowy format geometrii dla {block['name']}: {geom[:100]}...")
            return []

    if not geom or not isinstance(geom, dict):
        print(f"⚠️ Brak lub nieobsługiwany format geometrii dla {block['name']}")
        return []
...
def main():
    ...
    # Próbujemy użyć RPC 'get_blocks_geojson' dla optymalnego pobierania danych
    try:
        response = supabase.rpc("get_blocks_geojson").execute()
        blocks = response.data
        if not blocks:
            # Fallback na standardowy select, jeśli RPC nie zwraca nic
            blocks = supabase.table("vineyard_blocks").select("id, name, geom").execute().data
    except Exception as e:
        print(f"ℹ️ RPC 'get_blocks_geojson' nieudane ({e}), próba standardowego zapytania...")
        blocks = supabase.table("vineyard_blocks").select("id, name, geom").execute().data
