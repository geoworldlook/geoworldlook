...
if not all([SUPABASE_URL, SUPABASE_KEY, CLIENT_ID, CLIENT_SECRET]):
    print("⚠️ Uwaga: Brak wymaganych zmiennych środowiskowych w .env.local")
...
def fetch_block_data(token: str, block: dict, days_back: int = 365) -> list:
    # Używamy geometrii poligonu zamiast punktu
    geom = block['geom']
    ...
    print(f"📡 Pobieranie {days_back} dni telemetrii dla: {block['name']}...")
...
def main():
    if not supabase:
        print("❌ Supabase client not initialized. Check your .env.local file.")
        return

    print("🚀 Start procesu Backfill dla Poligonów...")
    
    # Pobieramy bloki winnicy
    # PostgREST domyślnie zwraca geometrię jako WKB (string Hex).
    # CDSE API wymaga formatu GeoJSON (dict).
    # Próbujemy pobrać dane, ST_AsGeoJSON(geom) w zapytaniu byłoby idealne.
