...
def fetch_vineyard_data(token: str, block: dict, days_back: int = 365) -> list:
    # Używamy geometrii bezpośrednio z bazy (GeoJSON)
    # Supabase może zwrócić geometrię jako string lub dict
    geom = block['geom']
...
def main():
    ...
    # Pobieramy działki używając RPC aby upewnić się, że dostajemy geometrię jako GeoJSON
    try:
    ...
    for block in blocks:
        stats = fetch_vineyard_data(token, block, days_back=365)
