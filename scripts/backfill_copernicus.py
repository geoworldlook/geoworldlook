...
# Evalscript obliczający NDVI oraz NDMI (B08-B11)/(B08+B11)
VINEYARD_EVALSCRIPT = """
//VERSION=3
function setup() {
  return {
...
def fetch_vineyard_stats(token: str, block: dict, days_back: int = 365) -> list:
    ...
    print(f"📡 Pobieranie telemetrii dla parceli: {block['name']}...")
...
def main():
    print("🚀 Start procesu Backfill dla Parceli Winnic...")
    
    # Pobieramy bloki winnic korzystając z naszej nowej funkcji RPC (lub bezpośrednio jeśli wolimy)
    # Dla skryptu backfill wygodniej może być pobrać bezpośrednio z tabeli lub użyć GeoJSON
    try:
    ...
    for feature in blocks_geojson['features']:
        ...
        stats = fetch_vineyard_stats(token, block_info, days_back=365)
