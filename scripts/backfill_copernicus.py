...
# Evalscript liczący NDVI, NDMI, Zachmurzenie i Maskę poprawności danych
EVALSCRIPT = """
//VERSION=3
function setup() {
  return {
...
def fetch_block_data(token: str, geom: dict, name: str, days_back: int = 365) -> list:
    ...
    print(f"📡 Pobieranie {days_back} dni telemetrii dla: {name}...")
...
def main():
    print("🚀 Start procesu Backfill...")
    
    # Pobieramy bloki winnic za pomocą funkcji RPC get_vineyard_blocks_geojson
    try:
        response = supabase.rpc("get_vineyard_blocks_geojson").execute()
        geojson = response.data
    except Exception as e:
        print(f"❌ Błąd RPC do pobierania działek: {e}")
        return
