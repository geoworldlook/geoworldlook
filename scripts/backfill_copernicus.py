...
if not all([SUPABASE_URL, SUPABASE_KEY, CLIENT_ID, CLIENT_SECRET]):
    print("⚠️ Uwaga: Brak wymaganych zmiennych w .env.local. Skrypt może nie działać w środowisku produkcyjnym.")
    # Nie rzucamy błędu, aby umożliwić budowanie frontendu bez pełnej konfiguracji API
...
# Evalscript obliczający NDVI i NDMI (stres wodny)
INDICES_EVALSCRIPT = """
//VERSION=3
function setup() {
  return {
...
def fetch_block_data(token: str, block: dict, days_back: int = 365) -> list:
    # Używamy rzeczywistej geometrii poligonu zamiast Bounding Boxa
    geom = block['geom']
    # Ensure geometry is a dict (Supabase might return it as a JSON string)
