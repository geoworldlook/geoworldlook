...
# Evalscript obliczający NDVI i NDMI (stres wodny) oraz maskujący chmury
EVALSCRIPT = """
//VERSION=3
function setup() {
  return {
...
def main():
    ...
    # Pobieramy działki z Supabase używając RPC aby otrzymać GeoJSON
    try:
        blocks = supabase.rpc("get_vineyard_blocks_geojson").execute().data
    except Exception as e:
        print(f"⚠️ Błąd RPC: {e}. Próba pobrania bezpośredniego...")
        # Fallback do bezpośredniego pobierania z rzutowaniem na GeoJSON w SQL
        # PostgREST nie zawsze wspiera zaawansowane selekty, więc RPC jest preferowane.
        # Jeśli RPC nie działa, rzutowanie w .select() może nie zadziałać z biblioteką supabase-py.
    ...
    for block in blocks:
        # Obsługa geometrii z RPC (geom_json) lub fallback (geom)
        if 'geom_json' not in block and 'geom' in block:
            # Próba rzutowania jeśli geom jest stringiem (WKB hex)
            # W idealnym świecie używamy RPC, które zwraca czysty JSON.
            if isinstance(block['geom'], str):
                 print(f"⚠️ Geometria dla {block['name']} jest w formacie WKB. Użyj funkcji RPC 'get_vineyard_blocks_geojson'.")
                 continue
            block['geom_json'] = block['geom']

        if 'geom_json' not in block:
            print(f"⚠️ Brak geometrii dla {block['name']}")
            continue
