...
# Evalscript do obliczania NDVI i NDMI oraz wyznaczania zachmurzenia na bazie SCL
EVALSCRIPT = """
//VERSION=3
function setup() {
  return {
...
def main():
    ...
    # Pobieramy działki w formacie GeoJSON przez funkcję RPC
    rpc_res = supabase.rpc("get_vineyard_blocks_geojson").execute()
    blocks_geojson = rpc_res.data

    features = blocks_geojson.get("features", []) if isinstance(blocks_geojson, dict) else []
    if not features:
    ...
    for feature in features:
        ...
        block_name = props.get("name", "Nieznana Parcela")
