...
def fetch_block_data(token: str, block: dict, days_back: int = 365) -> list:
    # block['geom'] to GeoJSON Polygon
    geom = block['geom']
    if isinstance(geom, str):
        geom = json.loads(geom)

    # Wyciągamy bbox z poligonu dla zapytania (uproszczone, API statystyk przyjmuje geometrię)
    # CDSE Statistics API pozwala na wysłanie poligonu w 'bounds'
...
def main():
    ...
    # Pobieramy działki z Supabase używając RPC, aby otrzymać geometrię w GeoJSON
    try:
        response = supabase.rpc("get_vineyard_blocks_geojson").execute()
        blocks_raw = response.data
    except Exception as e:
        print(f"❌ Błąd podczas wywołania RPC: {e}")
        return
    ...
    # Mapujemy wyniki RPC na format oczekiwany przez skrypt
    blocks = []
    for b in blocks_raw:
        blocks.append({
            "id": b['id'],
