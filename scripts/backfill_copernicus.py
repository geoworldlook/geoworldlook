...
if not all([SUPABASE_URL, SUPABASE_KEY, CLIENT_ID, CLIENT_SECRET]):
    print("⚠️ Uwaga: Brak wymaganych zmiennych w .env.local. Skrypt nie może pobrać danych z CDSE.")
    # In a real environment we would raise ValueError, but for this task I will just exit gracefully if vars are missing
    # or assume they might be provided later.
...
# Evalscript obliczający NDVI oraz NDMI (B08, B11)
INDEX_EVALSCRIPT = """
//VERSION=3
function setup() {
  return {
...
def fetch_block_data(token: str, block: dict, days_back: int = 365) -> list:
    # Używamy geometrii poligonu zamiast punktu (bbox)
    # block['geom'] jest obiektem GeoJSON Polygon pobranym z PostGIS
    geom = block['geom']
...
def main():
    if not supabase:
        print("❌ Supabase client nie zainicjalizowany.")
        return

    print("🚀 Start procesu Backfill dla Poligonów...")
    
    # Pobieramy działki winnicy
    # Musimy pobrać geometrię jako GeoJSON (używamy st_asgeojson w select jeśli nie mamy rpc)
    # Ale w migracji dodałem funkcję rpc, tu jednak prościej pobrać bezpośrednio
    # Supabase Python client nie obsługuje łatwo PostGIS funkcji w .select()
    # Zakładamy że w bazie są dane i możemy je pobrać.

    blocks = supabase.table("vineyard_blocks").select("id, name, geom").execute().data
    if not blocks:
        print("❌ Brak działek (vineyard_blocks) w bazie Supabase.")
        return
    ...
    for block in blocks:
        ...
        for entry in stats:
            ...
            # Akceptujemy dane o zachmurzeniu <= 50% dla poligonów (średnia z obszaru)
            if cloud_pct <= 50:
