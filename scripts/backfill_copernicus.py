...
# Evalscript pobierający pasma Red (B04), NIR (B08), SWIR (B11) i SCL dla wyliczania NDVI, NDMI i maskowania chmur
SATELLITE_EVALSCRIPT = """
//VERSION=3
function setup() {
  return {
...
def fetch_block_data(token: str, block: dict, days_back: int = 365) -> list:
    ...
    # Budujemy zapytanie o statystyki z dokładną geometrią wielokąta z bazy
    payload = {
        "input": {
            "bounds": {
                "geometry": block['geom'],
        ...
        "aggregation": {
            ...
            "evalscript": SATELLITE_EVALSCRIPT
        }
    }

    headers = {"Authorization": f"Bearer {token}", "Content-Type": "application/json"}
    print(f"📡 Pobieranie {days_back} dni telemetrii dla poligonu: {block['name']} ({block['area_ha']} ha)...")
...
def main():
    ...
    # Pobieramy działki za pomocą dedykowanej funkcji RPC zwracającej GeoJSON,
    # aby uniknąć problemów z pobieraniem surowych danych WKB
    try:
        rpc_resp = supabase.rpc("get_vineyard_blocks_geojson").execute()
        geojson = rpc_resp.data
    except Exception as e:
        print(f"❌ Krytyczny błąd pobierania danych z bazy: {e}")
        return
    ...
    if records_to_upsert:
        print(f"💾 Wysyłanie {len(records_to_upsert)} bezchmurnych odczytów poligonowych do Supabase...")
        try:
