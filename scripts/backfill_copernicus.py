...
def fetch_block_data(token: str, block_feature: dict, days_back: int = 365) -> list:
    geometry = block_feature['geometry']
    properties = block_feature['properties']
    ...
    print(f"📡 Pobieranie 365 dni telemetrii dla poligonu: {name}...")
    
    resp = requests.post(STATS_API_URL, json=payload, headers=headers)
    if resp.status_code != 200:
        print(f"⚠️ Błąd API CDSE dla {name}: {resp.text}")
        return []
...
def main():
    ...
    # Pobieramy geometrię działek wyłącznie przez RPC
    try:
    ...
    if not blocks_fc or "features" not in blocks_fc or not blocks_fc["features"]:
        print("❌ Brak poligonów działek w bazie Supabase (RPC zwrócił pustą listę).")
        return

    token = get_cdse_token()
    records_to_upsert = []

    for block in blocks_fc["features"]:
        block_id = block["properties"]["id"]
        name = block["properties"].get("name", "Unknown")
    ...
    if records_to_upsert:
        print(f"💾 Wysyłanie {len(records_to_upsert)} bezchmurnych odczytów poligonów do Supabase...")
        try:
            ...
            print(f"❌ Błąd zapisu statystyk do bazy: {e}")
    else:
