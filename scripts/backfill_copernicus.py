...
# Evalscript wyliczający NDVI (B08, B04) oraz NDMI (B08, B11)
EVALSCRIPT = """
//VERSION=3
function setup() {
  return {
...
def fetch_block_data(token: str, block_feature: dict, days_back: int = 365) -> list:
    block_id = block_feature.get('id') or block_feature.get('properties', {}).get('id')
    block_name = block_feature.get('properties', {}).get('name', 'Nieznana działka')
    geometry = block_feature.get('geometry')

    if not geometry:
        print(f"⚠️ Brak geometrii dla działki {block_name} ({block_id})")
        return []
    ...
    print(f"📡 Pobieranie {days_back} dni telemetrii dla: {block_name} ({block_id})...")
...
def main():
    ...
    # Pobieramy działki winnic w formacie GeoJSON za pomocą funkcji RPC
    res = supabase.rpc("get_vineyard_blocks_geojson").execute()
    geojson_data = res.data or {}
    features = geojson_data.get("features", [])

    if not features:
        print("❌ Brak działek w bazie Supabase (vineyard_blocks).")
        return
