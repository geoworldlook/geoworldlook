...
def fetch_block_data(token: str, block: dict, days_back: int = 365) -> list:
    # Używamy geometrii poligonu zamiast BBOX punktowego
    geom = block['geom']
...
def main():
    ...
    # Pobieramy bloki winnic z Supabase przy użyciu RPC lub bezpośrednio
    # Uwaga: wymagana funkcja ST_AsGeoJSON w bazie lub pobranie przez RPC
    try:
        blocks = supabase.rpc("get_vineyard_blocks_geojson").execute().data
    except:
        print("⚠️ RPC get_vineyard_blocks_geojson nie powiodło się, próba bezpośredniego pobrania...")
        blocks = supabase.table("vineyard_blocks").select("id, name").execute().data
        # Jeśli nie mamy geometrii, nie możemy pobrać danych z satelity
        print("❌ Brak geometrii dla bloków. Przerwanie.")
        return

    if not blocks:
        print("❌ Brak bloków w bazie Supabase.")
        return
    ...
    for block in blocks:
        # Sprawdzamy czy mamy geom_json (z RPC) lub geom
        geom = block.get('geom_json') or block.get('geom')
        if not geom:
            print(f"⏩ Pomijanie {block['name']} - brak geometrii.")
            continue

        stats = fetch_block_data(token, {**block, 'geom': geom}, days_back=365)
