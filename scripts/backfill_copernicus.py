...
def main():
    ...
    # Pobieramy bloki winnic z bazy (używając PostGIS st_asgeojson dla geom)
    # W pythonie supabase-py może nie obsłużyć st_asgeojson bezpośrednio w select,
    # więc najlepiej pobrać to przez RPC lub upewnić się że geom jest zwracany jako GeoJSON.
    # Zakładając że mamy dostęp do geometrii w formacie który CDSE zrozumie.
    
    # Najbezpieczniej użyć RPC lub zapytania które zwraca GeoJSON
    try:
