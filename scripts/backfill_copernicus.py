...
# Evalscript obliczający NDVI oraz NDMI
EVALSCRIPT = """
//VERSION=3
function setup() {
  return {
...
def fetch_block_stats(token: str, block: dict, days_back: int = 365) -> list:
    # Używamy rzeczywistej geometrii poligonu z bazy danych
    # PostGIS zwraca GeoJSON jako string lub słownik w zależności od query
    geom = block['geom']
    ...
    print(f"📡 Pobieranie statystyk dla działki: {block['name']}...")
...
def main():
    ...
    # Pobieramy działki z bazy (wymaga ST_AsGeoJSON dla geom)
    try:
        # Możemy użyć zapytania select z rzutowaniem do GeoJSON w Supabase-py
        # lub po prostu założyć że geom to już JSONB (jeśli tak to zapisaliśmy)
        # W naszej migracji geom jest typu geometry, więc musimy go skonwertować
        res = supabase.rpc("get_blocks_with_stats").execute()
        blocks = res.data
    ...
    for block in blocks:
        stats = fetch_block_stats(token, block, days_back=90) # Ostatnie 3 miesiące
        ...
        for entry in stats:
            ...
            # Akceptujemy tylko dane o zachmurzeniu <= 50% dla poligonów
            if cloud_pct <= 50:
