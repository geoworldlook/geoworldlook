...
# Evalscript dostosowany do obliczania NDVI oraz NDMI wraz z klasyfikacją chmur SCL
EVALSCRIPT = """
//VERSION=3
function setup() {
  return {
...
def fetch_block_data(token: str, block: dict, days_back: int = 365) -> list:
    ...
    block_name = block.get("properties", {}).get("name")
    geometry = block.get("geometry")
    
    if not geometry:
        print(f"⚠️ Pomijanie: {block_name} (Brak geometrii).")
        return []

    end_date = datetime.now()
    start_date = end_date - timedelta(days=days_back)

    # Budowanie żądania z użyciem geometrii wielokąta zamiast prostego bboxa
    payload = {
        "input": {
            "bounds": {
                "geometry": geometry,
    ...
    print(f"📡 Pobieranie {days_back} dni statystyk dla działki: {block_name} ({block_id})...")
...
def main():
    print("🚀 Start procesu Backfill (Poligony działek winnic)...")
    
    # Pobieramy działki jako GeoJSON przy użyciu funkcji RPC
    try:
        blocks_fc = supabase.rpc("get_vineyard_blocks_geojson", {}).execute().data
    except Exception as e:
        print(f"❌ Błąd wywołania RPC get_vineyard_blocks_geojson: {e}")
        return

    features = blocks_fc.get("features", []) if blocks_fc else []
    if not features:
        print("❌ Brak działek w bazie Supabase lub pusta kolekcja.")
        return
