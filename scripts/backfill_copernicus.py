...
# Evalscript z obsługą B04, B08, B11 oraz SCL (NDVI, NDMI, Cloud)
INDEXES_EVALSCRIPT = """
//VERSION=3
function setup() {
  return {
...
def get_bbox_from_geometry(geometry: dict) -> list:
...
def fetch_block_data(token: str, block_feature: dict, days_back: int = 365) -> list:
    ...
    block_name = properties.get("name", "Winnica")
...
def main():
    ...
    # Pobieramy działki w formacie GeoJSON via RPC
    response = supabase.rpc("get_vineyard_blocks_geojson").execute()
    geojson_data = response.data or {}
