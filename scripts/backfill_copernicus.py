...
if not all([SUPABASE_URL, SUPABASE_KEY, CLIENT_ID, CLIENT_SECRET]):
    print("⚠️ Ostrzeżenie: Brak wymaganych zmiennych w .env.local")
...
# Evalscript z obsługą NDVI (B08, B04), NDMI (B08, B11) i chmur (SCL)
INDICES_EVALSCRIPT = """
//VERSION=3
function setup() {
  return {
...
def main():
    ...
    if not supabase:
        print("❌ Supabase client nie jest zainicjalizowany.")
        return
    ...
    try:
        ...
        print(f"❌ Błąd pobierania działek z Supabase: {e}")
        return
    ...
    for feature in features:
        ...
        block_name = feature.get("properties", {}).get("name", "Działka")
