...
if not all([SUPABASE_URL, SUPABASE_KEY, CLIENT_ID, CLIENT_SECRET]):
    print("⚠️ Uwaga: Brak kompletnych zmiennych środowiskowych. Skrypt może nie działać w pełnym zakresie.")
...
def fetch_block_data(token: str, block: dict, days_back: int = 365) -> list:
    # block['geom'] jest już obiektem geometrycznym (Polygon)
    geom = block['geom']
...
def main():
    if not supabase:
        print("❌ Supabase nie skonfigurowane. Przerwanie.")
        return

    print("🚀 Start procesu Backfill dla Poligonów...")
    
    # Pobieramy bloki z Supabase (zakładamy że RPC lub prosty select zwraca geom)
    # W pythonie supabase-js zazwyczaj dostajemy geom jako dict jeśli to JSONB lub string jeśli to geometry
