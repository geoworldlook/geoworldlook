...
def fetch_block_data(token: str, block_name: str, geom: dict, days_back: int = 365) -> list:
...
def main():
    ...
    # Pobieramy działki za pomocą funkcji RPC get_vineyard_blocks_geojson
    try:
    ...
    if not geojson_data or not geojson_data.get("features"):
        print("❌ Brak działek (blocks) w bazie Supabase lub pusta FeatureCollection.")
        return
    ...
    for feature in features:
        ...
        block_name = props.get("name", "Unknown Block")
        geom = feature.get("geometry")

        if not block_id or not geom:
            print(f"⚠️ Pomiń niepoprawną działkę: {block_name}")
            continue

        stats = fetch_block_data(token, block_name, geom, days_back=365)
