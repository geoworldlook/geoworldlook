...
# Evalscript obliczający NDVI i NDMI oraz maskujący chmury
VINEYARD_EVALSCRIPT = """
//VERSION=3
function setup() {
  return {
...
def fetch_block_data(token: str, block: dict, days_back: int = 365) -> list:
    # Pobieramy geometrię bloku (Polygon)
    geom = block['geom']
    ...
    print(f"📡 Pobieranie 365 dni telemetrii dla bloku: {block['name']}...")
...
def main():
    ...
    # Pobieramy bloki winnicy z Supabase (w tym geometrię w formacie GeoJSON)
    # Wykorzystujemy st_asgeojson dla geometrii
    try:
        blocks = supabase.rpc("get_vineyard_blocks_geojson").execute().data
        if not blocks or 'features' not in blocks:
            # Próba bezpośredniego pobrania jeśli RPC nie działa/nie ma danych
            blocks_raw = supabase.table("vineyard_blocks").select("id, name, st_asgeojson(geom) as geom").execute().data
            blocks_list = []
            for b in blocks_raw:
                blocks_list.append({
                    'id': b['id'],
                    'name': b['name'],
                    'geom': json.loads(b['geom'])
                })
        else:
            blocks_list = []
