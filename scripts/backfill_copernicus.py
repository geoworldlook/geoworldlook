...
def extract_bbox(geom: dict) -> list:
    """Wyciąga bbox [min_lng, min_lat, max_lng, max_lat] z geometrii GeoJSON (Polygon)."""
    coords = []
    if geom.get("type") == "Polygon":
        coords = [pt for ring in geom.get("coordinates", []) for pt in ring]
    elif geom.get("type") == "MultiPolygon":
        coords = [pt for poly in geom.get("coordinates", []) for ring in poly for pt in ring]

    if not coords:
        return [15.5, 51.9, 15.51, 51.91]
...
def main():
    print("🚀 Start procesu Backfill dla działek winnicy (poligony)...")
    
    # Pobieramy działki z Supabase
    blocks = supabase.table("vineyard_blocks").select("*").execute().data
    if not blocks:
        print("❌ Brak działek winnicy (vineyard_blocks) w bazie Supabase.")
        return
