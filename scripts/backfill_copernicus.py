...
# Evalscript for NDVI, NDMI and Cloud Mask
...
def fetch_block_data(token: str, block: dict, days_back: int = 365) -> list:
    # Use the geometry from the block
    geom = block['geom']
...
def main():
    ...
    # Używamy RPC aby otrzymać geometrię w formacie GeoJSON
    try:
    ...
    if not blocks:
        print("⚠️ RPC nieudane, próbuję bezpośredniego zapytania...")
    ...
    for block in blocks:
        # Check if geom is a string (WKB) and needs conversion
        if isinstance(block['geom'], str):
            print(f"⚠️ Geometria dla {block['name']} jest w formacie tekstowym. Upewnij się, że to GeoJSON.")
            try:
