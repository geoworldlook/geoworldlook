...
# Evalscript rozszerzony o B11 dla NDMI i SCL dla chmur
...
def fetch_block_data(token: str, block: dict, days_back: int = 365) -> list:
    # Używamy RPC get_vineyard_blocks_geojson, aby pobrać geometrię (Polygon)
    # W skrypcie musimy jednak przekazać geometrię do API CDSE
    # Zakładamy, że block['geom'] jest już w formacie GeoJSON
...
def main():
    ...
    # Pobieramy bloki z Supabase (musimy mieć geometrię)
    # Ponieważ PostgREST nie zwraca GeoJSON bezpośrednio, użyjemy RPC lub castingu
    # Dla uproszczenia tutaj użyjemy zapytania, które oczekuje, że geom jest dostępny
    # UWAGA: W Supabase Python Client pobranie geom jako GeoJSON wymaga odpowiedniego przygotowania bazy
    # lub użycia raw query. Tutaj użyjemy RPC get_vineyard_blocks_geojson.

    rpc_resp = supabase.rpc("get_vineyard_blocks_geojson").execute()
    blocks_collection = rpc_resp.data
    if not blocks_collection or 'features' not in blocks_collection:
        print("❌ Brak działek w bazie Supabase lub błąd RPC.")
        return
    ...
    for feature in blocks_collection['features']:
        ...
        block_info = {'id': block_id, 'name': block_name, 'geom': geom}
        stats = fetch_block_data(token, block_info, days_back=365)
    ...
    if records_to_upsert:
        print(f"💾 Wysyłanie {len(records_to_upsert)} bezchmurnych odczytów do `vineyard_stats`...")
        try:
