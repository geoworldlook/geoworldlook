...
if not all([SUPABASE_URL, SUPABASE_KEY, CLIENT_ID, CLIENT_SECRET]):
    print("⚠️ Uwaga: Brak wymaganych zmiennych w .env.local. Skrypt może nie działać poprawnie w tym środowisku.")
...
def main():
    ...
    # Pobieramy działki z Supabase (geom musi być skonwertowany na GeoJSON)
    # W Pythonie biblioteka supabase-py może zwracać geometrię w różnym formacie,
    # najlepiej użyć RPC lub rzutowania w select.
    try:
        # Próbujemy pobrać dane przez RPC który zdefiniowaliśmy w migracji
        ...
        blocks = []
        for f in blocks_collection['features']:
            blocks.append({
                'id': f['properties']['id'],
        ...
        print(f"⚠️ Błąd przy pobieraniu działek: {e}")
        return
