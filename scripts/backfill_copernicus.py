...
if not all([SUPABASE_URL, SUPABASE_KEY, CLIENT_ID, CLIENT_SECRET]):
    print("⚠️ Uwaga: Brak niektórych zmiennych środowiskowych. Skrypt może nie działać poprawnie.")
...
def fetch_block_data(token: str, block: dict, days_back: int = 365) -> list:
    # block['geom'] should be a GeoJSON polygon
...
def main():
    ...
    # Pobieramy bloki z Supabase (geom jako GeoJSON za pomocą RPC)
    try:
        ...
        print(f"❌ Błąd podczas pobierania bloków via RPC: {e}")
        return
