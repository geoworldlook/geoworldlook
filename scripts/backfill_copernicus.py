...
def fetch_block_data(token: str, block_name: str, geom: dict, days_back: int = 365) -> list:
    ...
    print(f"📡 Pobieranie {days_back} dni telemetrii dla: {block_name}...")
...
def main():
    ...
    # Pobieramy działki z Supabase przez RPC aby otrzymać GeoJSON
    try:
        response = supabase.rpc("get_vineyard_blocks_geojson").execute()
        blocks_geojson = response.data
    except Exception as e:
        print(f"❌ Błąd wywołania RPC: {e}")
        return
    
    if not blocks_geojson:
