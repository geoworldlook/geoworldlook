...
# Evalscript liczący NDVI, NDMI, cloud cover i datamask dla Sentinel-2 L2A
EVALSCRIPT = """
//VERSION=3
function setup() {
  return {
...
def fetch_block_data(token: str, geom: dict, block_name: str, days_back: int = 365) -> list:
...
def main():
    ...
    # Pobieramy poligonowe działki przy użyciu RPC get_vineyard_blocks_geojson
    try:
    ...
    if not geojson or not geojson.get("features"):
        print("❌ Brak działek w bazie Supabase (GeoJSON puste).")
        return
    ...
    for feature in features:
        ...
        if not geom or not block_id:
            print(f"⚠️ Pomiń niekompletny feature: {properties}")
            continue

        stats = fetch_block_data(token, geom, block_name, days_back=365)
        ...
        for entry in stats:
            ...
            # Wymagamy prawidłowego odczytu NDVI
