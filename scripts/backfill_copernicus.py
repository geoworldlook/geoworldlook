...
# Evalscript obliczający NDVI i NDMI (B08, B04, B11)
INDEX_EVALSCRIPT = """
//VERSION=3
function setup() {
  return {
...
def fetch_block_stats(token: str, block: dict, days_back: int = 365) -> list:
    # block['geom'] zawiera geometrię jako obiekt GeoJSON (dzięki RPC lub PostGIS)
    # Jeśli geom jest stringiem (JSON), musimy go sparsować
    geom = block['geom']
    ...
    print(f"📡 Pobieranie statystyk dla poligonu: {block['name']}...")
...
def main():
    print("🚀 Start procesu Backfill (Polygon Edition)...")
    
    # Pobieramy bloki winnicy z bazy (tym razem z geometrią)
    # Możemy użyć .select("*") bo PostGIS w Supabase domyślnie zwraca geometrię (często jako GeoJSON string lub obiekt)
