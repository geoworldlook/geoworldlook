...
# Evalscript obliczający NDVI, NDMI oraz maskujący chmury (SCL)
EVALSCRIPT = """
//VERSION=3
function setup() {
  return {
...
def fetch_block_data(token: str, block: dict, days_back: int = 365) -> list:
    # Używamy geometrii poligonu przekazanej jako słownik (GeoJSON)
    geom = block.get('geometry') or block.get('geom')
    if isinstance(geom, str):
        try:
            geom = json.loads(geom)
        except json.JSONDecodeError:
            print(f"⚠️ Nie można sparsować geometrii dla {block.get('name')}")
            return []
    
    if not geom:
        print(f"⚠️ Brak geometrii dla {block.get('name')}")
        return []
...
def main():
    ...
    # Pobieramy działki z Supabase korzystając z RPC, które zwraca GeoJSON
    try:
    ...
    if not geojson or 'features' not in geojson:
        print("❌ Brak działek (lub błędny format) w bazie Supabase.")
        return
