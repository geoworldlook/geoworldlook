...
# Evalscript obliczający NDVI oraz NDMI na podstawie Sentinel-2 (L2A)
# Bands: B04 (Red), B08 (NIR), B11 (SWIR), SCL (Scene Classification Layer)
...
def fetch_block_data(token: str, block: dict, days_back: int = 365) -> list:
    ...
    print(f"📡 Pobieranie {days_back} dni telemetrii dla działki: {block['properties']['name']}...")
...
def main():
    print("🚀 Start procesu Copernicus Polygon Backfill...")
    
    # Pobieramy działki jako GeoJSON FeatureCollection za pomocą RPC
    try:
        rpc_response = supabase.rpc("get_vineyard_blocks_geojson").execute()
        geojson_data = rpc_response.data
    except Exception as e:
        print(f"❌ Błąd pobierania działek z RPC: {e}")
        return

    if not geojson_data or 'features' not in geojson_data or not geojson_data['features']:
        print("❌ Brak działek (blocks) w bazie Supabase.")
        return

    blocks = geojson_data['features']
