...
if not all([SUPABASE_URL, SUPABASE_KEY, CLIENT_ID, CLIENT_SECRET]):
    # Since during local development or test suites environment variables might be missing,
    # let's output a soft warning instead of hard crash, so other scripts or compilation do not fail,
    # but still let's keep the verification check friendly.
    print("⚠️ Uwaga: Brak wymaganych zmiennych w .env.local")

# We wrap Supabase client initialization in a safe try-except
...
# Evalscript liczący zarówno NDVI, jak i NDMI
S2_EVALSCRIPT = """
//VERSION=3
function setup() {
  return {
...
def fetch_block_stats(token: str, geometry: dict, block_name: str, days_back: int = 365) -> list:
...
def main():
    ...
    if not supabase:
        print("❌ Brak sprawnej konfiguracji Supabase. Przerywanie backfillu.")
        return

    # Pobieramy poligony poprzez funkcję RPC (zwraca GeoJSON FeatureCollection)
    try:
    ...
    if not geojson or not geojson.get("features"):
        print("❌ Brak poligonów w bazie Supabase lub pusta kolekcja.")
        return

    features = geojson["features"]
    print(f"ℹ️ Wykryto {len(features)} działek winnicy do przetworzenia.")

    try:
        token = get_cdse_token()
    except Exception as e:
        print(f"❌ Błąd autoryzacji w CDSE: {e}")
        return
    ...
    for feature in features:
        ...
        block_name = properties.get("name", "Nieznana działka")
        geometry = feature.get("geometry")

        if not block_id or not geometry:
            print(f"⚠️ Pomijanie niekompletnego rekordu: {block_name}")
            continue

        stats = fetch_block_stats(token, geometry, block_name, days_back=365)
    ...
    if records_to_upsert:
        print(f"💾 Wysyłanie {len(records_to_upsert)} bezchmurnych odczytów dla działek do Supabase...")
        try:
            ...
            print(f"❌ Błąd zapisu statystyk do Supabase: {e}")
    else:
