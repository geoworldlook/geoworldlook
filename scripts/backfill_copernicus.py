...
if not all([SUPABASE_URL, SUPABASE_KEY, CLIENT_ID, CLIENT_SECRET]):
    raise ValueError("Critical error: Missing required variables in .env.local")
...
# Updated Evalscript to calculate both NDVI and NDMI
...
def fetch_block_data(token: str, block: dict, days_back: int = 365) -> list:
    # Użycie geometrii poligonu bezpośrednio z bazy danych
    geom = block['geom']
    ...
    if resp.status_code != 200:
        print(f"⚠️ CDSE API error for {block['name']}: {resp.text}")
        return []
...
def main():
    ...
    # Pobranie działek winnicy z Supabase
    ...
    for block in blocks:
        ...
        for entry in stats:
            ...
            # Accept data with cloud cover <= 40%
            if cloud_pct <= 40:
