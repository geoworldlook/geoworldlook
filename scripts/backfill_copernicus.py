...
def fetch_block_stats(token: str, block_geojson: dict, days_back: int = 90) -> list:
    geometry = block_geojson['geometry']
    ...
    print(f"📡 Pobieranie statystyk dla działki: {block_geojson['properties']['name']}...")
    
    resp = requests.post(STATS_API_URL, json=payload, headers=headers)
    if resp.status_code != 200:
        print(f"⚠️ Błąd API CDSE: {resp.text}")
        return []
...
def main():
    ...
    for feature in geojson['features']:
        ...
        stats = fetch_block_stats(token, feature, days_back=90)
    ...
    if records_to_upsert:
        print(f"💾 Wysyłanie {len(records_to_upsert)} rekordów do Supabase...")
        try:
