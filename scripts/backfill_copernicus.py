...
# Evalscript obliczający NDVI, NDMI oraz maskę chmur
SENTINEL_EVALSCRIPT = """
//VERSION=3
function setup() {
  return {
...
def main():
    ...
    # Uwaga: geom musi być pobrane jako GeoJSON. Jeśli używamy PostgREST bezpośrednio,
    # może być wymagane rzutowanie w select lub użycie RPC.
    # Zakładamy że 'geom' w tabeli jest typu geometry.
    try:
        # W nowszych wersjach Supabase Python / PostgREST,
        # pobieranie geometrii może wymagać st_asgeojson.
        # Jeśli powyższe nie zadziała, użyjemy RPC.
    ...
    for block in blocks:
        # Jeśli geom jest stringiem (WKB), musimy go sparsować.
        # Ale tutaj zakładamy że dostajemy słownik GeoJSON.
