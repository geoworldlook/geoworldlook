...
def fetch_block_data(token: str, block: dict, days_back: int = 365) -> list:
    # Use the geometry coordinates of the block
    geom = block.get('geom')
    if not geom:
...
def main():
    ...
    # Pobieramy działki za pomocą RPC get_vineyard_blocks_geojson, co zapobiega problemom z WKB
    try:
        rpc_res = supabase.rpc("get_vineyard_blocks_geojson").execute()
        feature_coll = rpc_res.data
    except Exception as e:
        print(f"❌ Błąd RPC podczas pobierania działek: {e}")
        return

    if not feature_coll or "features" not in feature_coll:
        print("❌ Brak działek w bazie Supabase lub błędny GeoJSON.")
        return
    ...
    if not blocks:
        print("❌ Brak działek do przetworzenia.")
        return
