...
if not all([SUPABASE_URL, SUPABASE_KEY, CLIENT_ID, CLIENT_SECRET]):
    # Note: We fallback to print warning instead of hard crash so python tests / pre-commits
    # do not fail if env files are empty in build environments.
    print("⚠️ Uwaga: Brak niektórych zmiennych w .env.local (Wymagane do wykonania rzeczywistego backfillu).")
...
def main():
    ...
    if not all([SUPABASE_URL, SUPABASE_KEY, CLIENT_ID, CLIENT_SECRET]):
        print("❌ Przerwano backfill: brak wymaganych zmiennych środowiskowych.")
        return

    supabase: Client = create_client(SUPABASE_URL, SUPABASE_KEY)

    # Pobieramy działki w formacie GeoJSON za pomocą funkcji RPC
    print("📡 Pobieranie definicji działek z Supabase...")
    try:
    ...
    for feature in features:
        ...
        block_name = properties.get("name", "Nienazwana działka")

        if not geometry or not block_id:
            print(f"⚠️ Pomijanie niekompletnego feature: {properties}")
            continue
