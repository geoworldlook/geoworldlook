...
function evaluatePixel(sample) {
  ...
  let denom_ndmi = sample.B08 + sample.B11;
  let ndmi = (denom_ndmi !== 0) ? (sample.B08 - sample.B11) / denom_ndmi : 0;
...
def fetch_block_data(token: str, block_feature: dict, days_back: int = 365) -> list:
    ...
    properties = block_feature.get('properties', {})
...
def main():
    ...
    # Pobieramy działki jako GeoJSON FeatureCollection z Supabase RPC
    ...
    if not geojson_data or not isinstance(geojson_data, dict):
        print("❌ Brak poprawnych danych GeoJSON z Supabase RPC.")
        return
    ...
    for feature in features:
        block_id = feature.get("properties", {}).get("id") or feature.get("id")
        if not block_id:
            continue
