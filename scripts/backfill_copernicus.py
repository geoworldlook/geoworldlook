...
if not all([SUPABASE_URL, SUPABASE_KEY, CLIENT_ID, CLIENT_SECRET]):
    print("⚠️ Uwaga: Brak wymaganych zmiennych w .env.local. Skrypt może nie działać w pełnym zakresie.")
...
# Evalscript obliczający NDVI i NDMI dla poligonów
EVAL_SCRIPT = """
//VERSION=3
function setup() {
  return {
...
def fetch_block_data(token: str, block: dict, days_back: int = 365) -> list:
    ...
    geometry = block['geom'] # Zakładamy że otrzymujemy GeoJSON geometry
    ...
    print(f"📡 Pobieranie {days_back} dni telemetrii dla bloku: {block['name']}...")
...
def main():
    ...
    # Pobieramy bloki z Supabase (zakładamy RPC który zwraca listę z geometrią lub selekcję z ST_AsGeoJSON)
    # Dla uproszczenia pobieramy bezpośrednio z tabeli i mapujemy geometrię
    try:
        # Pobranie bloków z zakodowaną geometrią GeoJSON
        blocks_data = supabase.rpc('get_blocks_geojson').execute().data
        if not blocks_data or 'features' not in blocks_data:
            print("❌ Brak bloków w bazie Supabase lub błąd RPC.")
            return
