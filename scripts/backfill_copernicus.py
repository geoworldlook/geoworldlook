...
def fetch_block_stats(token: str, block: dict, days_back: int = 365) -> list:
    ...
    print(f"📡 Pobieranie telemetrii dla działki: {block['name']}...")
...
def main():
    ...
    # Pobieramy działki za pomocą funkcji RPC, aby otrzymać geometrię w formacie GeoJSON
    try:
    ...
    if records_to_upsert:
        print(f"💾 Wysyłanie {len(records_to_upsert)} odczytów do Supabase (tabela vineyard_stats)...")
        try:
            # Dzielimy na paczki po 500 rekordów dla bezpieczeństwa
            for i in range(0, len(records_to_upsert), 500):
                chunk = records_to_upsert[i:i + 500]
