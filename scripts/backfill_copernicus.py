...
if not all([SUPABASE_URL, SUPABASE_KEY, CLIENT_ID, CLIENT_SECRET]):
    print("⚠️ Uwaga: Brak niektórych zmiennych środowiskowych. Skrypt może nie działać w pełni.")
...
# Evalscript obliczający NDVI (roślinność) i NDMI (stres wodny)
...
def fetch_block_data(token: str, block: dict, days_back: int = 365) -> list:
    # Używamy geometrii poligonu zamiast BBOX wokół punktu
    geom = block['geom']
    ...
    payload = {
        ...
        "aggregation": {
            ...
            "evalscript": VEGETATION_EVALSCRIPT
        }
    }
...
def main():
    if not supabase:
        print("❌ Brak połączenia z Supabase. Przerwanie.")
        return

    print("🚀 Start procesu Backfill dla Poligonów...")
    
    # Pobieramy działki z Supabase wraz z geometrią (ST_AsGeoJSON)
    # Warto użyć rpc lub widoku jeśli bezpośrednie pobieranie geom jest problematyczne
    blocks = supabase.table("vineyard_blocks").select("id, name, ST_AsGeoJSON(geom) as geom").execute().data

    if not blocks:
        # Fallback jeśli bezpośrednie ST_AsGeoJSON nie zadziała w .select()
        # W Supabase Python client może być wymagane pobranie wszystkich pól i liczenie na to że geom wróci jako GeoJSON
        try:
            blocks = supabase.table("vineyard_blocks").select("*").execute().data
        except:
            print("❌ Nie udało się pobrać działek.")
            return
    ...
    try:
        ...
        print(f"❌ Błąd autoryzacji CDSE: {e}")
        return
    ...
    for block in blocks:
        ...
        for entry in stats:
            ...
            # Akceptujemy tylko dane o zachmurzeniu <= 50% dla poligonów (większa tolerancja)
            if cloud_pct <= 50:
    ...
    if records_to_upsert:
        print(f"💾 Wysyłanie {len(records_to_upsert)} odczytów do `vineyard_stats`...")
        try:
            # Dzielenie na paczki po 1000 rekordów
            for i in range(0, len(records_to_upsert), 1000):
                batch = records_to_upsert[i:i+1000]
                supabase.table("vineyard_stats").upsert(batch, on_conflict="block_id, date").execute()
            print("✅ Backfill zakończony sukcesem!")
        except Exception as e:
            print(f"❌ Błąd zapisu: {e}")
    else:
        print("⚠️ Brak nowych danych do zapisu.")
