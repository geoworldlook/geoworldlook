...
def fetch_block_data(token: str, block: dict, days_back: int = 365) -> list:
    # Pobieramy geometrię (GeoJSON) z bazy
    # geom jest już obiektem lub stringiem JSON
    geom = block['geom']
...
def main():
    print("🚀 Start procesu Backfill (Winnice)...")
    
    # Pobieramy działki z Supabase (z geometrią jako GeoJSON)
    # Wymaga ST_AsGeoJSON(geom) jeśli robimy to przez select, ale supabase-js pobiera geom jako obiekt
    # Jeśli geom jest typu geometry, zostanie zwrócony jako string WKB/EWKB w niektórych klientach,
    # ale Supabase PostgREST zazwyczaj zwraca GeoJSON dla typów geometry.
