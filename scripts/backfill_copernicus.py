...
# Evalscript dla NDVI i NDMI (B08-B11)/(B08+B11)
SENTINEL_EVALSCRIPT = """
//VERSION=3
function setup() {
  return {
...
def fetch_block_data(token: str, block: dict, days_back: int = 365) -> list:
    ...
    # Supabase RPC zwraca GeoJSON, który Python-supabase mapuje na słownik
...
def main():
    ...
    # Pobieramy działki jako GeoJSON przez RPC
    try:
        ...
        if not blocks_collection or not blocks_collection.get('features'):
            ...
            return
        blocks = blocks_collection['features']
