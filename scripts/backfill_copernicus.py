...
# Evalscript obliczający NDVI i NDMI oraz maskę chmur
EVALSCRIPT = """
//VERSION=3
function setup() {
  return {
...
def fetch_block_data(token: str, block: dict, days_back: int = 365) -> list:
    ...
    # Zakładamy, że geom jest już w formacie słownika GeoJSON lub musi zostać sparsowany
    geom = block['geom']
    ...
    print(f"📡 Pobieranie {days_back} dni statystyk dla działki: {block['name']}...")
...
def main():
    ...
    # Pobieramy działki winnicy z Supabase
    # Uwaga: Może być wymagany RPC lub widok, aby pobrać geometrię jako GeoJSON
    # Na razie zakładamy, że standardowy select zadziała (zależy od konfiguracji PostgREST)
    try:
    ...
    if not blocks:
        print("❌ Brak działek w tabeli vineyard_blocks.")
        return
