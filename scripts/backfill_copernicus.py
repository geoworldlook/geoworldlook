...
def calculate_bbox(geometry: dict) -> list:
    """Oblicza bounding box [min_lng, min_lat, max_lng, max_lat] dla geometrii GeoJSON Polygon."""
...
def fetch_block_data(token: str, block: dict, days_back: int = 365) -> list:
    ...
    bbox = calculate_bbox(geometry) if geometry else [0, 0, 0, 0]
    ...
    block_name = block.get("properties", {}).get("name", block.get("id"))
...
def main():
    ...
    # Pobieramy działki w formacie GeoJSON przez RPC
    try:
        rpc_resp = supabase.rpc("get_vineyard_blocks_geojson").execute()
        geojson = rpc_resp.data if rpc_resp.data else {}
        blocks = geojson.get("features", [])
    except Exception as e:
        print(f"⚠️ Błąd wywołania RPC get_vineyard_blocks_geojson: {e}")
        blocks = []
