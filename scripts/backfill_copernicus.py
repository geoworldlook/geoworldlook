...
def fetch_block_data(token: str, block: dict, days_back: int = 365) -> list:
    # block['geom'] jest w formacie GeoJSON (z PostGIS)
    # Supabase Python client może zwrócić to jako słownik lub string
    geom = block['geom']
...
def main():
    ...
    # Pobieramy działki z Supabase (geom jako GeoJSON)
    # Używamy .select("*, geom.to_geojson()") jeśli to możliwe,
    # ale w Python client po prostu pobieramy wszystko.
    # Jeśli geom nie jest automatycznie GeoJSON, upewnij się że tabela ma odpowiednie uprawnienia.
