...
def main():
    ...
    # Pobieramy bloki winnicy wraz z geometriami (używając RPC jeśli dostępne lub bezpośrednio)
    try:
        # Próbujemy użyć RPC by dostać GeoJSON
        blocks = supabase.rpc("get_vineyard_blocks_geojson").execute().data
    except Exception:
        print("⚠️ RPC get_vineyard_blocks_geojson nie powiodło się, pobieram surowe dane...")
    ...
    for block in blocks:
        if not block.get('geom'):
            print(f"⚠️ Brak geometrii dla bloku {block['name']}, pomijam.")
            continue
