...
# Evalscript obliczający NDVI i NDMI (wymaga B04, B08, B11)
INDEX_EVALSCRIPT = """
//VERSION=3
function setup() {
  return {
...
def main():
    ...
    # Używamy RPC lub bezpośrednio, ale potrzebujemy geometrii w formacie dict (GeoJSON)
    # Dla uproszczenia tutaj pobierzemy wszystkie rekordy.
    # Uwaga: Supabase/PostgREST może zwracać geometrię jako string WKB, jeśli nie użyjemy RPC.
    # Użyjmy RPC get_vineyard_blocks_geojson, aby dostać czyste GeoJSON.
    
    rpc_resp = supabase.rpc("get_vineyard_blocks_geojson").execute()
    feature_collection = rpc_resp.data
