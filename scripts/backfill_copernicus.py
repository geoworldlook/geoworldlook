...
def fetch_block_data(token: str, block: dict, days_back: int = 365) -> list:
    # block['geom'] to obiekt GeoJSON Polygon
    geometry = block['geom']
    ...
    print(f"📡 Pobieranie {days_back} dni statystyk dla poligonu: {block['name']}...")
...
def main():
    ...
    # Pobieramy bloki winnicy z Supabase
    # Uwaga: PostgREST domyślnie zwraca geometrię jako WKB (hex string).
    # Aby otrzymać GeoJSON, najlepiej użyć RPC lub specyficznego rzutowania w select (jeśli wspierane).
    # Tutaj zakładamy, że mamy funkcję pomocniczą lub rzutujemy.
    
    # Wariant z RPC:
    try:
        response = supabase.rpc("get_vineyard_blocks_geojson").execute()
        blocks_collection = response.data
        blocks = []
        if blocks_collection and 'features' in blocks_collection:
            for feature in blocks_collection['features']:
                blocks.append({
        ...
        print(f"⚠️ Nie udało się pobrać bloków przez RPC: {e}")
        # Próba standardowego pobrania (może wymagać dodatkowej obróbki geom)
    ...
    for block in blocks:
        # Jeśli geom jest stringiem (WKB), pomijamy w tym uproszczonym skrypcie
        # (wymagałoby shapely/pyproj do konwersji)
        if isinstance(block['geom'], str):
            print(f"⚠️ Pomijam {block['name']} - geometria w formacie WKB (wymaga GeoJSON).")
            continue
    ...
    if records_to_upsert:
        print(f"💾 Wysyłanie {len(records_to_upsert)} bezchmurnych rekordów do vineyard_stats...")
        try:
