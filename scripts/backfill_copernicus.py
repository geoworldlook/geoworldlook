...
# Evalscript pobierający pasma czerwone (B04), bliskiej podczerwieni (B08), krótkofalowej podczerwieni (B11) i SCL
EVALSCRIPT = """
//VERSION=3
function setup() {
  return {
...
def fetch_block_data(token: str, block: dict, days_back: int = 365) -> list:
    # Pobieramy geometrię bezpośrednio z GeoJSON przekazanego przez RPC
    geom = block.get("geometry")
    if not geom:
        print(f"⚠️ Brak geometrii dla działki: {block['properties'].get('name')}")
        return []
    ...
    block_name = block["properties"].get("name", "Nieznana działka")
    print(f"📡 Pobieranie 365 dni telemetrii dla poligonu: {block_name}...")
...
def main():
    ...
    # Wywołanie funkcji RPC w celu pobrania działek w formacie GeoJSON
    try:
        response = supabase.rpc("get_vineyard_blocks_geojson").execute()
        geojson = response.data
    except Exception as e:
        print(f"❌ Błąd RPC podczas pobierania działek: {e}")
        return

    if not geojson or "features" not in geojson or not geojson["features"]:
        print("❌ Brak działek (poligonów) w bazie Supabase lub pusta kolekcja.")
        return
    ...
    for block in features:
        block_id = block["properties"].get("id")
        block_name = block["properties"].get("name", "Nieznana działka")

        if not block_id:
            print(f"⚠️ Brak ID w właściwościach działki: {block_name}")
            continue
