...
def fetch_block_data(token: str, block_geometry: dict, block_name: str, days_back: int = 365) -> list:
...
def main():
    ...
    # Pobieramy działki jako GeoJSON FeatureCollection za pomocą RPC get_vineyard_blocks_geojson
    try:
    ...
    for feature in features:
        ...
        block_name = props.get("name")
        block_geometry = feature.get("geometry")

        if not block_id or not block_geometry:
            print(f"⚠️ Pomiń niekompletny feature: {props}")
            continue

        stats = fetch_block_data(token, block_geometry, block_name, days_back=365)
