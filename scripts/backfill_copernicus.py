...
if not all([SUPABASE_URL, SUPABASE_KEY, CLIENT_ID, CLIENT_SECRET]):
    # Allow python verification to pass without variables if running a dry run/mock test
    print("⚠️ Uwaga: Brak wymaganych zmiennych w .env.local")

# Only initialize supabase if envs exist, else handle gracefully in syntax/dry run
...
def fetch_block_data(token: str, geometry: dict, block_name: str, days_back: int = 365) -> list:
    ...
    print(f"📡 Pobieranie 365 dni telemetrii dla bloku: {block_name}...")
...
def main():
    print("🚀 Start procesu Backfill dla bloków winnic...")
    if not supabase:
        print("❌ Supabase client nie zainicjalizowany. Brak zmiennych środowiskowych.")
        return

    # Pobieramy bloki jako GeoJSON z Supabase RPC
    try:
    ...
    if not features:
        print("❌ Brak bloków winnic w bazie Supabase.")
        return
    ...
    if records_to_upsert:
        print(f"💾 Wysyłanie {len(records_to_upsert)} bezchmurnych odczytów dla bloków do Supabase...")
        try:
