...
# Evalscript obliczający NDVI oraz NDMI (używając NIR B08 i SWIR B11)
EVALSCRIPT = """
//VERSION=3
function setup() {
  return {
...
def main():
    print("🚀 Start procesu Backfill Poligonów...")
    
    # Pobieramy działki z Supabase
    # Ponieważ geom jest w formacie PostGIS, musimy go pobrać jako GeoJSON lub obsłużyć WKB
    # Najprościej użyć RPC get_vineyard_blocks_geojson i sparsować go
    try:
    ...
    if records_to_upsert:
        ...
        try:
            # Chunking upserts if too many records
            for i in range(0, len(records_to_upsert), 1000):
                chunk = records_to_upsert[i:i + 1000]
                supabase.table("vineyard_stats").upsert(chunk, on_conflict="block_id, date").execute()
