...
# Evalscript z NDVI i NDMI (B08-B11)/(B08+B11)
VINE_EVALSCRIPT = """
//VERSION=3
function setup() {
  return {
...
def fetch_block_data(token: str, block: dict, days_back: int = 365) -> list:
    # Używamy geometrii bezpośrednio z GeoJSON
    geom = block['geom']
...
def main():
    ...
    # Pobieramy działki przez RPC by mieć gotowy GeoJSON
    try:
        rpc_resp = supabase.rpc('get_vineyard_blocks_geojson').execute()
        blocks_fc = rpc_resp.data
